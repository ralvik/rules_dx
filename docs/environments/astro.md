...
Astro contributes provider-derived plans to the private environment
...
The `astro_env_plan` rule in `astro/env/plan.bzl` plans one `astro_*`
wrapper target from its preserved `JsInfo` transitive sources plus
`QualitySourcesInfo` direct sources, emitting `AstroEnvPlanInfo` plus a
JSON plan. Binaries and tests share the same closure through
`data`/runfiles. Pinned by `astro/env/plan_tests.bzl` and exercised by
`astro/env:hello_lib_plan` over `//astro/tests/fixtures/hello:hello_lib`.
