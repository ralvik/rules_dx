...
Vue contributes provider-derived plans to the private environment collection
...
The `vue_env_plan` rule in `vue/env/plan.bzl` plans one `vue_*` wrapper
target from its preserved `JsInfo` transitive sources plus
`QualitySourcesInfo` direct sources, emitting `VueEnvPlanInfo` plus a JSON
plan. Binaries and tests share the same closure through `data`/runfiles.
Pinned by `vue/env/plan_tests.bzl` and exercised by
`vue/env:hello_lib_plan` over `//vue/tests/fixtures/hello:hello_lib`.
