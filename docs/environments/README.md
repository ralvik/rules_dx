...
`java`, `kotlin`, `scala`, `csharp`, `fsharp`, `cc`, `ruby`, `powershell`) contributes a provider-derived
...
C/C++, Ruby, and PowerShell plans are owned by [Go](go.md), [Java](java.md),
[Kotlin](kotlin.md), [Scala](scala.md), [C#](csharp.md), [F#](fsharp.md),
[C/C++](cc.md), [Ruby](ruby.md), and [PowerShell](powershell.md); Vue, Svelte, Astro, and MDX plans are owned by [Vue](vue.md),
...
Ruby mapping is delivered in the Ruby track and PowerShell mapping is delivered
provisionally under issue #972.
Deferred/excluded env record is decided by [ADR 0032](../decisions/0032-ruby-powershell-bandit-swift.md);
no `swift/` env plan lands here yet.
