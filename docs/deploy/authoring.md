...
x86_64 qualified-built-here; Linux arm64 plus macOS arm64 plus Windows
x86_64 qualified with per-host evidence under issue #815: Platform-qualified
per the [support matrix](../product/support-matrix.md) plus per-host
`sbom-provenance` release evidence per the
[support matrix](../product/support-matrix.md#status-lifecycle); macOS x86_64
Not planned per #976 with no cell). Draft-only ceiling enforced, owner approval required.
...
- Matrix (`matrix.bzl`): four cells, seed `dx-linux-x86_64`
  qualified-built-here, Linux arm64 plus macOS arm64 plus Windows x86_64
  qualified with per-host evidence under issue #815 (Platform-qualified per
  ADR 0014 plus per-host `sbom-provenance` release evidence; macOS x86_64
  Not planned per #976 with no cell).
