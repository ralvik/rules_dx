...
| O62 | Decide that vendored `.bazelrc` execution flags are Bazel execution policy, distinct from the quality-tool behavioral preset [ADR 0011](decisions/0011-configuration-composition.md) rejects; adopt `bazelrc-preset.bzl` vendoring with per-flag review and owned `extra_presets`. | M05 | Narrow provisional pick, flagged for review: upstream preset content arrives as reviewed diffs only, project overrides stay explicit, `user.bazelrc` stays last; consumer delivery deferred to M27. See [M05](milestones/M05-direct-bazel-dogfood.md). |
