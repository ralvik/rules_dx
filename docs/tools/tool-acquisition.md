...
delivered work under closed #796-#800 (successors to closed #416-#420), not qualified
pins or platform support. Recheck latest stable and verify actual bytes when adding each adapter.
The JVM cohort rows below are owned by closed #796 (successor to closed #416; live successor to closed #307 for the `java`/`kotlin` classes); versions are observations, not pins.
The Scala + .NET cohort rows below are owned by closed #797 (successor to closed #417; live successor to closed #307 for the `scala`/`csharp`/`fsharp` classes); versions are
observations, not pins.
The Native cohort rows below are owned by closed #798 (successor to closed #418; live successor to closed #307 for the `c`/`cpp`/`go` classes); versions are
observations, not pins.
The Structured cohort rows below are owned by closed #799 (successor to closed #419; live successor to closed #307 for the `protobuf`/`qml` classes); versions are
observations, not pins.
The Interpreted/file-family cohort rows below are owned by closed #800 (successor to closed #420; live successor
...
classes; `protobuf`/`qml` stay owned by closed #799 (successor to closed #419) and are cross-linked, not
...
| Error Prone | [v2.50.0 release](https://github.com/google/error-prone/releases/tag/v2.50.0) (Jun 2026) | Javac plugin following the qualified JDK baseline (version coupled to the JDK, tracked under closed #796, successor to closed #416); no structured diagnostics upstream, so javac-diagnostic parsing plus per-target declared `error-prone.patch` outputs (`-XepPatchChecks` plus `-XepPatchLocation:<declared-dir>`; `IN_PLACE` rejected as it mutates inputs and breaks sandboxing) are decided with fixtures under issue #491 itemized here, not silently dropped; adapter dispatch stays owned under closed #796 (successor to closed #416). |
...
| Scalafix | [v0.14.7 release](https://github.com/scalacenter/scalafix/releases/tag/v0.14.7) (Jun 2026) | Semantic-rule artifacts over the one managed JDK cohort plus the Scala Maven-lock story; semantic rules additionally need semanticdb plus classpath wiring per the adapter-input notes. No machine-readable CLI output upstream, so console-parse is rejected and wire via `scalafix.interfaces.ScalafixMainCallback` is required, decided under issue #490 with `scala/tests/fixtures/scalafix/` evidence (cohort stays owned under closed #797, successor to closed #417), itemized here, not silently dropped. Maintainer acquisition must establish and record byte identity. |
...
| FSharpLint | [0.27.0 release](https://github.com/fsprojects/FSharpLint/releases) (Jun 2026; targets .NET 8.0) | Exact official tool package over the one managed .NET runtime cohort; wire via the `FSharpLint.Core` library API (`FSharpLint.Application.Lint` with `ReceivedWarning` callback) is decided with fixtures under issue #493 itemized here, not silently dropped (standard plus `-f msbuild` console-parse is rejected as lossy with shared-stream info lines; diagnostics via the library record, cohort stays owned under closed #797 (successor to closed #417). Default ruleset with formatting rules off (Fantomas owns formatting) stays a provisional native-config input until qualified. Maintainer acquisition must establish and record byte identity. |
...
| clang-tidy | [hermetic-llvm v0.8.19](https://github.com/hermeticbuild/hermetic-llvm/blob/v0.8.19/README.md) (inspected release; LLVM tool targets) | Authoritative-toolchain class: resolves from the qualified hermetic-llvm LLVM distribution's tool targets, no separate acquisition. clang-tidy needs compile-commands context — target-coupled wiring versus check-only stays tracked under closed #798 (successor to closed #418) itemized here, not silently dropped. Version follows the qualified toolchain pin. |
...
| RuboCop | [releases](https://github.com/rubocop/rubocop/releases) (1.91.0 observed Sep 2026; [RubyGems](https://rubygems.org/gems/rubocop)) | Release-assembled Ruby closure member (gem plus transitive closure over the assembled Ruby runtime); consumer never runs Bundler or any installer. `--format json` versus text-parse stays tracked under closed #800 (successor to closed #420). Bundle contents, lock inputs, manifest, SBOM, licenses, constituent provenance, and adapter qualification remain pending under #800 (successor to closed #420); maintainer assembly must establish and record them and recheck latest stable when adding the adapter. |
| StandardRB | [releases](https://github.com/standardrb/standard/releases) (1.56.0 observed Jul 2026; [RubyGems](https://rubygems.org/gems/standard)) | Release-assembled Ruby closure member over the same assembled Ruby runtime (`standardrb` binary wrapping RuboCop with an unconfigurable ruleset); consumer never runs Bundler or any installer. Whole-file rewrite with check/diff mode; exact fix wiring stays tracked under closed #800 (successor to closed #420). Bundle contents, lock inputs, and adapter qualification remain pending under #800 (successor to closed #420); recheck latest stable when adding the adapter. |
| PSScriptAnalyzer | [releases](https://github.com/PowerShell/PSScriptAnalyzer/releases) (1.25.0 observed Mar 2026; [PowerShell Gallery](https://www.powershellgallery.com/packages/PSScriptAnalyzer)) | Exact upstream module package imported by explicit path over a portable `pwsh` runtime; no consumer runs `Install-Module`. Console-parse versus library-API binding stays tracked under closed #800 (successor to closed #420; per the adapter-input notes). Maintainer acquisition must establish and record byte identity and recheck latest stable when adding the adapter. |
| pwsh | [releases](https://github.com/PowerShell/PowerShell/releases) (7.6.5 LTS observed Aug 2026; 7.5.10 stable) | Portable runtime for the PSScriptAnalyzer module (per-platform binary archives, execution-platform lazy); exact runtime identity and platform artifact qualification remain pending under #800 (successor to closed #420). Maintainer acquisition must establish and record byte identity. |
...
| jsonnetfmt | [go-jsonnet releases](https://github.com/google/go-jsonnet/releases) (v0.22.0 observed Mar 2026; C++ [jsonnet v0.21.0](https://github.com/google/jsonnet/releases)) | Checksummed standalone release artifact; go-jsonnet versus C++ implementation choice stays tracked under closed #800 (successor to closed #420). Whole-file rewrite with check/diff mode. Maintainer acquisition must establish and record byte identity and recheck latest stable when adding the adapter. |
...
| djlint | [releases](https://github.com/djlint/djLint/releases) (v1.45.0 observed Sep 2026; [PyPI](https://pypi.org/project/djlint)) | Private wheel-only Python graph member over the shared managed Python runtime (no sdist fallback, no pip subprocess on the consumer path). `--lint` versus `--reformat` wiring stays tracked under closed #800 (successor to closed #420). Maintainer acquisition must establish and record byte identity and recheck latest stable when adding the adapter. |
| Stylelint | [releases](https://github.com/stylelint/stylelint/releases) (17.14.1 observed Jul 2026; [npm](https://www.npmjs.com/package/stylelint)) | Private pure-JavaScript graph member over the shared managed Node runtime (standalone-artifact alternative stays tracked under closed #800 (successor to closed #420)). `--formatter json` shape stays an unproven mapping owned by #800 (successor to closed #420). Maintainer acquisition must establish and record byte identity and recheck latest stable when adding the adapter. |
...
| modfmt | [releases](https://github.com/joshdk/modfmt/releases/tag/v0.4.0) (modfmt v0.4.0 observed Sep 2025; formatter for `go.mod` plus `go.work`, MIT) | Checksummed standalone release artifact for `go.mod` formatting; whole-file rewrite with check/diff mode. Maintainer acquisition must establish and record byte identity; upstream identity resolved seed-only under closed #582 to github.com/joshdk/modfmt v0.4.0, digests stay owned under closed #800 (successor to closed #420). |
...
delivered work under closed #416-#420 (successors closed #796-#800).
...
(closed #505) before implementation;
...
remaining asset qualification in delivered work under closed #796-#800 (successors to closed #416-#420).
...
itemized work stay owned under closed #796).
...
adapter-input notes (console-parse is rejected, wire via `scalafix.interfaces.ScalafixMainCallback` plus target-coupled `--classpath` plus `--sourceroot` plus `--semanticdb-targetroots` with sandbox-apply-and-diff, decided under issue #490 with `scala/tests/fixtures/scalafix/` evidence, recorded here, not silent). Exact versions plus rule-sets qualified seed-only under issue #486
...
digests plus adapter mappings stay owned under #797 (successor to closed #417)
(live successor to closed #307 for this cohort) and no
adapter claims `scala` yet (tracked under closed #797, successor to closed #417).
...
FSharpLint wire via `FSharpLint.Application.Lint` with `ReceivedWarning` decided under issue #493
with `fsharp/tests/fixtures/fsharplint/` evidence (standard plus `-f msbuild` console-parse is rejected,
recorded here, not silent));
runtime compatibility bounds plus adapter mappings stay owned under #797 (successor to closed #417)
(live successor to closed #307
for this cohort) and no adapter claims `csharp` or
`fsharp` yet (tracked under closed #797, successor to closed #417).
...
(tracked under closed #800, successor to closed #420).
...
wiring versus check-only stays tracked under closed #798 (successor to closed #418) and is recorded
...
digests plus adapter mappings stay owned under closed #798 (successor to closed #418) (live successor to
closed #307 for the `c`/`cpp` classes) and no adapter claims `c` or `cpp` yet
(tracked under closed #798, successor to closed #418). C/C++ MSVC-interop and SDK licensing stay with
...
(`bazel run //tools/ci:native_quality_qualification` with
`cc/tests/fixtures/native_quality/pins.bzl` over upstream built-in defaults
with no hidden preset); digests plus adapter mappings stay owned under #798 (successor to closed #418)
(live successor to closed #307 for the `go` class) and no adapter claims `go`
yet (tracked under closed #798, successor to closed #418).
...
modfmt v0.4.0 resolved seed-only under closed #582, living at head rejected);
digests plus adapter mappings stay owned under closed #800 (successor to closed #420; live successor to closed #307 for the `cue`/`jsonnet`/`pkl`/`terraform`/`yaml`/`text`/`go_module` classes) and no adapter claims `cue` yet (tracked under closed #800, successor to closed #420). `protobuf`/`qml` stay owned by closed #419, never double-claimed here.
...
digests plus adapter mappings stay owned under closed #800 (successor to closed #420) (live successor to
closed #307 for the `html_template`/`yaml` classes) and no adapter claims
`html_template` or `yaml` yet (tracked under closed #800, successor to closed #420).
...
(`bazel run //tools/ci:file_family_defaults_qualification` with
`quality/tests/fixtures/file_family_quality/pins.bzl` over upstream built-in
defaults with no hidden preset; prettier-plugin-gherkin 4.0.0 plus @prettier/plugin-xml 3.4.2 resolved seed-only under closed #582, rechecked latest stable at implementation);
digests plus adapter mappings stay owned under closed #800 (successor to closed #420; live successor to closed #307 for the `css`/`gherkin`/`sql`/`xml` classes) and no adapter claims `css` yet (tracked under closed #800, successor to closed #420).
