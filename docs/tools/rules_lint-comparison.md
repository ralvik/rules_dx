...
Measured seed-host slice (issue #86 slice 3): `perf/rules_lint_results.json` records server-warm quality-only numbers (report-not-gate, no parity claim), proven by `bazel test //perf:rules_lint_results_test` (13/13: pins, quality-sample aquery, timing present, regen digest match). Synth tree 200 files / 20 dirty, tree `0a9fbc0f…8283de`, generation ~79ms; quality sample `//perf:corpus` lint+format yields 2 actions (`DxRealQualityLint`, `DxRealQualityFormat`); runner test `//quality/runner:quality_runner_test` aquery 6 actions, cold-ish (with `--nocache_test_results`) ~1812ms executing 1/1 vs warm ~990ms executing 0/1 (cache-hit evidence); peak memory explicitly uncollected (gap, not claimed). Post-shutdown cold + `rules_lint`-side numbers follow next.
