# Java Generation Contract

Java follows the [common contract](common.md). Admitted to v1 by
[ADR 0019](../decisions/0019-first-release-additional-foundations.md) on the
provisional `rules_java` upstream; no switch is approved here.

Wrappers in `java/rules/defs.bzl` preserve `JavaInfo` and add
`QualitySourcesInfo`. The extension lives in `gazelle/java/` with
parser/naming/lang fixtures plus focused tests. Hello builds in
`java/tests/fixtures/hello/` consume the wrappers.

## Sources And Ownership

One directory holds one reusable `java_library` named after the directory
basename with sorted non-test `.java` sources. `*Test.java` files are never
library sources; handwritten `java_test` owns them. Thin `java_binary`
entries are never inferred, and a directory mixing a library with a
`main`-defining source fails so the owner splits it first. Mixed-package
directories fail. Exact naming, test, and main rules live in
`gazelle/java/naming.go` and `gazelle/java/parser.go`, pinned by their
fixtures.
