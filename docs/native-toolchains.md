...
macOS x86_64 is Not planned per #976 with no qualified closure on the same
as-built stack: CI builds, tests, and gates coverage natively on
`macos-14` (arm64) runners through the pinned upstream toolchains (macOS x86_64 Not planned per #976 with no Intel runner)
(`rules_rust`, `aspect_rules_py`, `aspect_rules_js`/`aspect_rules_ts`,
plus the admitted foundations' rulesets), which already resolve
`macos/x86_64` acquisition is Not planned per #976 with no CI runner. `macos-13`
retirement history stays docs-only; gaps recorded
...
`macos_x86_64` artifacts were removed per #976 (Not planned); only `macos_arm64` plus Linux plus Windows artifacts are delivered (issue #616) as bounded
delivery-class records; the
...
| Rust | `rules_rs v0.0.109` and its pinned patched `rules_rust` | Retain the preferred foundation and provider identities in ADR 0013; do not compose a second independent Rust rules graph. Dated deferral per issue #961 (2026-09-22): patch inventory plus upstreaming plus upgrade owned by issue #916 (commit `e9dd49f`, see Inspected Identities below). |
...
| Windows native backend | `toolchains_msvc` at `8e2aa4624bbb5a53a94f135e90995f307875d1ad`, clang-cl + lld-link + Microsoft STL | First Windows-hosted candidate with existing no-install toolchains and the required STL family. Dated deferral per issue #961 (2026-09-22): exact commit pin under ADR 0008 with migration to a published stable release on every currency pass. Prototype acquisition and coverage gaps block adoption unchanged. |
...
| macOS x86_64 | Not planned | Never planned for support per #976 with no CI runner or scope; clean `unsupported_platform` refusal |
...
to obtain a larger cross-build table. Cross routes are qualified seed-only under issue #504
(`cc/tests/fixtures/cross_routes/pins.bzl` via `bazel run //tools/ci:cross_routes_qualification`):
Linux cross is the first priority, not a mandate to build every target from every host, with native
rows qualified under issues #410-#414 and same-arch musl closures qualified under issue #411; Linux
cross-arch stays in the first cohort with matching native target execution plus separate cache and
remote evidence; macOS arm64 to Linux plus macOS x86_64 stays optional expansion only if bounded
upstream configuration suffices. Backends stay provisional; no `Supported` claim.
...
Rosetta, remote execution and deployment-floor testing are distinct evidence. Cross-building alone
as execution proof plus emulation plus Rosetta plus remote execution as target execution plus
compiler-target availability as route proof plus cross-host Windows inference stay rejected under
issue #504 with per-row separate cache and remote evidence. The inspected
...
rejected. The backend stays provisional; floors qualified seed-only under
issue #500, coverage qualified seed-only under issue #501; acquisition rights are qualified seed-only under
...
Per-host strict laziness with override-matrix plus symlink-privilege UX is
qualified seed-only under issue #917
(`cc/tests/fixtures/laziness_host/pins.bzl` via `bazel run
//tools/ci:laziness_host_qualification`): per-host no EULA/SDK payload when
adding an unused foundation across the required hosts via the
single-foundation adopt-* consumers, default plus root-override graphs stay
lazy, symlink probe before mutation with Enterprise-unsupported refusal and
no fallback. Seed-executed with static per-host pins. Backend stays
provisional; no `Supported` claim.
...
Backends stay provisional; floors qualified seed-only under issue #500,
coverage qualified seed-only under issue #501; windows transport is qualified seed-only under issue #497 below;
prebuilt interop is qualified seed-only
under issue #498 below; the SQLite plus OpenSSL plus ring plus bindgen
plus CXX corpus is qualified seed-only under issue #499 below; floors
are qualified seed-only under issue #500 below;
no `Supported` claim.
...
under issue #499, floors qualified seed-only under issue #500, coverage
qualified seed-only under issue #501;
no `Supported` claim.
...
corpus is qualified seed-only under issue #499 below; floors qualified
...
Backends stay provisional; windows transport qualified seed-only under issue
#497 above; floors qualified seed-only under issue #500, coverage
qualified seed-only under issue #501;
no `Supported` claim.

Deployment plus execution floors are qualified seed-only under issue #500
(`cc/tests/fixtures/deployment_floors/pins.bzl` via `bazel run
//tools/ci:deployment_floors_qualification`): glibc `2.28` symbol floor
plus musl `1.2.6` static closure plus macOS deployment `14.0` starting
point plus MacOSX26.5 SDK identity plus Windows retail `/MD` CRT with
Microsoft STL/UCRT/VCRuntime plus MSVC `14.50.35717` plus redist
`14.50.35710` plus SDK package `10.0.26100.7705` identities, with
oldest-target and current-host fixtures run separately plus compiler,
clangd and bindgen loader dependencies plus Apple extracted SDK framework
subset; SDK version is not deployment floor; `/MT` plus debug CRT are not
assumed interchangeable; unpinned floors rejected. Backends stay
provisional; windows transport qualified seed-only under issue #497 above;
linux corpus qualified seed-only under issue #499 above; coverage qualified
...
explicitly before admission rather than treating compiler flags as complete support. Strict ownership
and resolution is qualified seed-only under issue #503 (`cc/tests/fixtures/strict_generation/pins.bzl`
via `bazel run //tools/ci:strict_generation_qualification`): quoted basename identities resolve
strictly or fail, angle includes never produce edges, ambiguous basenames fail, macro includes
contribute no edge with no synthesized ignore, comments plus literals plus raw strings inert,
authoritative metadata is the local index plus exact mappings never the module index, generated
headers require exact mappings, test grouping keeps `*_test` out, assembly stays undiscovered,
modules plus header units plus PCH stay handwritten, union of literals with no `select()`,
deterministic names with collisions failing, stale cleanup through normal merge; loose generation rejected
plus first-candidate plus warn-and-omit plus disappearing angle plus log parsing plus replacement
preprocessor plus flags-as-support plus inferred producers plus platform selections rejected.
Backends stay provisional; floors qualified seed-only under issue #500, coverage qualified seed-only
under issue #501, linux corpus qualified seed-only under issue #499; no `Supported` claim.
...
symlink-only environment contract. A narrow action-derived snapshot adaptation with managed
host-native clangd, generated-output materialization and multiple header contexts is qualified
seed-only under issue #754 (`cc/tests/fixtures/cpp_snapshot/pins.bzl` via
`bazel run //tools/ci:cpp_snapshot_qualification`). Do not infer
...
sole route. The C++ snapshot slice is qualified seed-only under issue #754
(`cc/tests/fixtures/cpp_snapshot/pins.bzl` via `bazel run //tools/ci:cpp_snapshot_qualification`).
Prefer an upstream exact-target discovery extension over
...
`bazel run //tools/ci:exact_target_qualification`); C++ snapshot is qualified seed-only under issue #754
(action-derived `CppCompile` via `aquery` plus generated sources plus multi-context headers plus managed host tools
plus Bazel-9 compatibility; pinned in `cc/tests/fixtures/cpp_snapshot/pins.bzl` via
`bazel run //tools/ci:cpp_snapshot_qualification`). Exact current stable-stack compose is frozen
...
`bazel run //tools/ci:exact_target_qualification`; C++ snapshot is pinned by
`bazel run //tools/ci:cpp_snapshot_qualification`; stable-stack compose is pinned by
...
| Does strict laziness hold per-host in the consumer graph with override-matrix plus symlink-privilege UX? | Qualified seed-only under issue #917: per-host no EULA/SDK payload when adding an unused foundation across the required hosts via the single-foundation adopt-* consumers, default plus root-override graphs stay lazy with no special CLI policy, Bzlmod version-resolution boundary with deferred failure never proving laziness; symlink probe before mutation with Developer Mode guidance, Enterprise hosts that cannot grant it unsupported with failure before mutation, junction/launcher/copy fallback rejected; pinned in `cc/tests/fixtures/laziness_host/pins.bzl` via `bazel run //tools/ci:laziness_host_qualification`. Seed-executed with static per-host pins. Backend stays provisional; no `Supported` claim. | issue #917 |
...
| Which prebuilt native libraries interoperate? | Qualified seed-only under issue #498: explicit Windows STL/CRT/linker/library combos (clang-cl plus Microsoft STL plus retail `/MD` with static `.lib` plus import `.lib` plus DLL shapes, `cl.exe` compat as diagnosis only) plus Linux explicit dynamic libstdc++ comparison on Linux glibc only plus ordinary object linking without cross-language LTO plus shared-runtime plus exceptions plus RTTI plus allocation-ownership plus ABI boundaries, pinned in `cc/tests/fixtures/prebuilt_interop/pins.bzl` via `bazel run //tools/ci:prebuilt_interop_qualification`; host-to-target plus target execution qualify separately with mixed Rust/C/C++ composition, compiler-target availability alone plus LLVM ancestry alone plus single-combo proof rejected. Windows x86_64 qualified (issue #414) with representative prebuilt-MSVC fixtures incl mixed Rust/C/C++ stays the host record without double-claiming here. Backends stay provisional; windows transport qualified seed-only under issue #497, linux corpus qualified seed-only under issue #499, floors qualified seed-only under issue #500, coverage qualified seed-only under issue #501. | issue #498 |
| Are both Linux profiles complete? | Qualified seed-only under issue #499: native arm64 glibc builds/tests plus per-cell coverage landed (issue #410); static-musl closures plus per-cell coverage for both musl profiles landed (issue #411, Rust musl std plus exec/target separation, CI cross-builds with per-profile cache scopes, no union; dynamic musl explicitly out of scope); the SQLite plus OpenSSL plus ring plus bindgen plus CXX corpus is qualified seed-only (`cc/tests/fixtures/linux_corpus/pins.bzl` via `bazel run //tools/ci:linux_corpus_qualification`, single-crate proof rejected, native only). Backends stay provisional; windows transport qualified seed-only under issue #497; floors qualified seed-only under issue #500; PIE plus ELF-dependency plus glibc-symbol bounded seed-only under issue #505 (`cc/tests/fixtures/remediation_bounds/pins.bzl` via `bazel run //tools/ci:remediation_bounds_qualification`, existing upstream constraints with no new infra), cross-build completeness qualified seed-only under issue #504 (`cc/tests/fixtures/cross_routes/pins.bzl` via `bazel run //tools/ci:cross_routes_qualification`), coverage qualified seed-only under issue #501. | issue #499 |
| Can Windows acquisition be immutable and lazy? | Qualified seed-only under issue #495: fixed-manifest plus package-index inputs pinned with the toolchains_msvc head plus windows_support `v0.4.1` package identities (`cc/tests/fixtures/windows_acquisition/pins.bzl` via `bazel run //tools/ci:windows_acquisition_qualification`); mutable fetch rejected; laziness proven (adding the module requires no acceptance and fetches no restricted payloads, unrelated workflows stay green without acceptance, deferred failure never proves laziness). Backend stays provisional; acquisition rights qualified seed-only under issue #496, windows transport qualified seed-only under issue #497, prebuilt interop qualified seed-only under issue #498, linux corpus qualified seed-only under issue #499, floors qualified seed-only under issue #500, coverage stays owned under issue #501. | issue #495 |
| Are Apple/Microsoft acquisition and cache rights adequate? | Qualified seed-only under issue #496: hermetic-llvm `v0.8.19` MacOSX26.5 extraction vs [Apple SDK agreement](https://www.apple.com/legal/sla/docs/xcode.pdf) with hosted execution not authorizing separate extraction or unrestricted caching, toolchains_msvc plus windows_support `v0.4.1` identities with deliberate EULA repository-env never automatic plus SDK EULA gap reviewed not inferred, usage vs redistribution reviewed separately (acceptance is not redistribution permission), mirrors plus redistribution plus internal caches plus remote workers as license-approved boundaries separately from download success, official download not permission, assume rights rejected; pinned in `cc/tests/fixtures/acquisition_rights/pins.bzl` via `bazel run //tools/ci:acquisition_rights_qualification`. Backend stays provisional; windows transport qualified seed-only under issue #497, prebuilt interop qualified seed-only under issue #498, linux corpus qualified seed-only under issue #499, floors qualified seed-only under issue #500, coverage stays owned under issue #501. | issue #496 |
| Can the kept CC opt-out execute successfully? | Qualified seed-only under issue #471: kept opt-out succeeds for pure-Rust scripts on stock `rules_rust` 0.74.0 (sysroot `rust-lld` fallback plus `no_cc` stubs, `rust/tests/fixtures/cc_optout/` via `bazel run //tools/ci:cc_optout_qualification`); script compilation inputs stay distinct from execution inputs. | issue #471 |
| Can third-party scripts retain a declared hermetic closure? | Decided hermetic under issue #472: global shell-env False in `.bazelrc` with narrow per-crate annotation opt-in (zero opt-ins); hostile PATH, tool discovery and additional declared tools pinned by `bazel run //tools/ci:shell_env_qualification` plus [Rust Generation](generation/rust.md#build-scripts). | issue #472 |
| Does Windows native transport preserve all inputs and ABI selection? | Qualified seed-only under issue #497: explicit ABI constraint fix (toolchains_msvc registration plus rules_rs LLVM MSVC ABI identity, GNU/GNULVM rejected) plus `/external:I` vs `/imsvc` plus `.lib`/`.obj` bare-path handling (patched rules_rust rebasing baseline) with batch wrappers plus response files plus spaces plus SDK libraries plus cc-rs discovery/assembly plus proc-macro DLLs on declared-input fixtures without host Visual Studio state (`cc/tests/fixtures/windows_transport/pins.bzl` via `bazel run //tools/ci:windows_transport_qualification`); compiler-target availability alone is not proof. Backend stays provisional; rights plus interop qualified seed-only under issues #496/#498, linux corpus qualified seed-only under issue #499, floors qualified seed-only under issue #500, coverage stays owned under issue #501. | issue #497 |
| Which prebuilt native libraries interoperate? | Qualified seed-only under issue #498: explicit Windows STL/CRT/linker/library combos (clang-cl plus Microsoft STL plus retail `/MD` with static `.lib` plus import `.lib` plus DLL shapes, `cl.exe` compat as diagnosis only) plus Linux explicit dynamic libstdc++ comparison on Linux glibc only plus ordinary object linking without cross-language LTO plus shared-runtime plus exceptions plus RTTI plus allocation-ownership plus ABI boundaries, pinned in `cc/tests/fixtures/prebuilt_interop/pins.bzl` via `bazel run //tools/ci:prebuilt_interop_qualification`; host-to-target plus target execution qualify separately with mixed Rust/C/C++ composition, compiler-target availability alone plus LLVM ancestry alone plus single-combo proof rejected. Windows x86_64 qualified (issue #414) with representative prebuilt-MSVC fixtures incl mixed Rust/C/C++ stays the host record without double-claiming here. Backends stay provisional; windows transport qualified seed-only under issue #497, linux corpus qualified seed-only under issue #499, floors qualified seed-only under issue #500, coverage stays owned under issue #501. | issue #498 |
| Are both Linux profiles complete? | Qualified seed-only under issue #499: native arm64 glibc builds/tests plus per-cell coverage landed (issue #410); static-musl closures plus per-cell coverage for both musl profiles landed (issue #411, Rust musl std plus exec/target separation, CI cross-builds with per-profile cache scopes, no union; dynamic musl explicitly out of scope); the SQLite plus OpenSSL plus ring plus bindgen plus CXX corpus is qualified seed-only (`cc/tests/fixtures/linux_corpus/pins.bzl` via `bazel run //tools/ci:linux_corpus_qualification`, single-crate proof rejected, native only). Backends stay provisional; windows transport qualified seed-only under issue #497; floors qualified seed-only under issue #500; PIE plus ELF-dependency plus glibc-symbol bounded seed-only under issue #505 (`cc/tests/fixtures/remediation_bounds/pins.bzl` via `bazel run //tools/ci:remediation_bounds_qualification`, existing upstream constraints with no new infra), cross-build completeness qualified seed-only under issue #504 (`cc/tests/fixtures/cross_routes/pins.bzl` via `bazel run //tools/ci:cross_routes_qualification`), coverage qualified seed-only under issue #501. | issue #499 |
| Which deployment and execution floors are supportable? | Qualified seed-only under issue #500: glibc `2.28` symbol floor plus musl `1.2.6` static closure plus macOS deployment `14.0` starting point plus MacOSX26.5 SDK identity plus Windows retail `/MD` CRT with Microsoft STL/UCRT/VCRuntime plus MSVC `14.50.35717` plus redist `14.50.35710` plus SDK package `10.0.26100.7705` identities, pinned in `cc/tests/fixtures/deployment_floors/pins.bzl` via `bazel run //tools/ci:deployment_floors_qualification`; oldest-target and current-host fixtures run separately with matching native target execution, never merged; compiler, clangd and bindgen loader dependencies inspected separately with the minimal archive not proof; Apple extracted SDK framework subset checked; SDK version is not deployment floor; `/MT` plus debug CRT are not assumed interchangeable; unpinned floors rejected. macOS arm64 native is qualified (issue #412) with oldest-OS execution plus framework completeness plus licensing as gates; macOS x86_64 Not planned per #976. Windows x86_64 native is qualified (issue #414) with `/MD` retail dynamic CRT as the starting point. Backends stay provisional; coverage qualified seed-only under issue #501; no `Supported` claim. | issue #500 |
| Can every executable first-party line be accounted for? | Qualified seed-only under issue #501: Rust-only plus C/C++-only plus mixed/DLL LCOV with missed-line tests, coverage-tool version pairing, native ignores and denominator validation, pinned in `cc/tests/fixtures/lcov_accounting/pins.bzl` via `bazel run //tools/ci:lcov_accounting_qualification`; unaccounted lines plus ignored collection failures rejected. Backends stay provisional; floors qualified seed-only under issue #500. | issue #501 |
...
| Can public Cargo metadata represent every generated target? | Qualified seed-only under issue #502: features (nonempty `required-features` stays opt-in with kept handwritten `testonly` plus explicit `crate_features`, no unconditional target or feature activation, narrow upstream admissibility export), build-script metadata (conventional `build.rs` plus explicit `package.build` plus `build = false` authoritative, `[package] version` feeds script `version` plus single-version checks, script sees only `[build-dependencies]`, hermetic defaults, exec action never during generate, `BuildScriptInfo` as narrow export), target kinds (ordinary lib/bin/test/proc-macro/sole `cdylib`/sole `staticlib` only through the stable public mapping, `dylib`/multiple/unknown with kept route, no coercion, examples/benches only explicit ordinary-binary with affixes), ownership (crate root owns module tree, integration-test roots own separately, one source one owner, sibling lib plus script edges, mirrored path deps, public libs plus private bins/tests/scripts), public shape (checked-in declarations plus local rule index plus public `@crates//:crates.bzl` `crate_deps`/`aliases` with parent-dir-plus-package `package_name`), pinned in `rust/tests/fixtures/cargo_metadata/pins.bzl` with the `Cargo.toml` plus `cargo_metadata.expected` pair via `bazel run //tools/ci:cargo_metadata_qualification`; ad-hoc metadata rejected (private serialized graph, `Cargo.Bazel.lock`, `cargo-bazel.json`, private maps). Backends stay provisional; floors qualified seed-only under issue #500, coverage qualified seed-only under issue #501 (qualified seed-only under issue #502). | issue #502 |
| Can generation satisfy strict ownership and resolution cheaply? | Qualified seed-only under issue #503: quoted plus angle plus ambiguous plus macro include fixtures with basename identities resolving strictly or failing, authoritative local-index plus exact-mapping metadata never the module index, test grouping keeping `*_test` out, generated headers requiring exact mappings, assembly staying undiscovered, modules plus header units plus PCH staying handwritten with union of literals plus deterministic names plus merge lifecycle, pinned in `cc/tests/fixtures/strict_generation/pins.bzl` via `bazel run //tools/ci:strict_generation_qualification`; loose generation rejected, first-candidate plus warn-and-omit plus disappearing angle plus log parsing plus replacement preprocessor plus flags-as-support plus inferred producers plus platform selections rejected. Backends stay provisional; floors qualified seed-only under issue #500, coverage qualified seed-only under issue #501, linux corpus qualified seed-only under issue #499. | issue #503 |
| Can IDE setup preserve exact context and projection contracts? | Qualified seed-only under issue #475 for Rust exact-target discovery (resolver-owned exact labels to `gen_rust_project`/`flycheck` TARGETS, `Path`/`Buildfile` widening plus project-owned graph plus `RustAnalyzerInfo` rejected, hello exact-isolation pair plus `rust/tests/fixtures/discovery/pins.bzl` via `bazel run //tools/ci:exact_target_qualification`); qualified seed-only under issue #754 for C++ action-derived snapshot plus generated sources plus multi-context headers plus managed host tools plus Bazel-9 compatibility (exact owners as snapshot input, `CppCompile` via `aquery` never infer compile commands from `CcInfo`, generated-output materialization through exact mappings, `hello.h` multi-context kept apart, managed host-native clangd never unrestricted query-driver, Bazel 9.2.0 plus rules_cc 0.2.22, pinned in `cc/tests/fixtures/cpp_snapshot/pins.bzl` via `bazel run //tools/ci:cpp_snapshot_qualification`; refresh Bazel plus preprocessors plus workspace writes plus continued-after-failures plus package-wide widening rejected). | issues #475, #754 |
| Which cross routes actually work and execute? | Qualified seed-only under issue #504: Linux cross is the first priority, not a mandate to build every target from every host, pinned in `cc/tests/fixtures/cross_routes/pins.bzl` via `bazel run //tools/ci:cross_routes_qualification`; Linux x86_64 plus Linux arm64 execution each cover Linux x86_64 and arm64 glibc plus static musl as the first cohort with native rows qualified under issues #410-#414 and same-arch musl closures qualified under issue #411; Linux cross-arch stays in the cohort with matching native target execution plus separate cache and remote evidence; macOS arm64 to Linux plus macOS x86_64 stays optional expansion only if bounded upstream configuration suffices; Linux/macOS-to-Windows plus Linux/Windows-to-macOS plus Windows-to-Linux plus Windows arm64 stay outside the cohort without weakening required native workflows; execution platform plus native target execution plus emulation/Rosetta/remote/floor distinctions plus per-row cache and remote evidence preserved; all-cross mandate rejected; native only with no Windows/macOS cross-host claim. Backends stay provisional; floors qualified seed-only under issue #500, coverage qualified seed-only under issue #501, linux corpus qualified seed-only under issue #499; no `Supported` claim. | issue #504 |
| Is the remediation bounded enough for admission? | Qualified seed-only under issue #505: every native defect is reproduced with estimate plus actual owner plus patch plus upstream-issue plus upgrade tracking plus complete-workflow evidence, pinned in `cc/tests/fixtures/remediation_bounds/pins.bzl` via `bazel run //tools/ci:remediation_bounds_qualification`; focused tested pinned patches plus bounded integration plus small missing pieces with clearly bounded scope plus upstreaming preferred plus established-alternative comparison with explicit contract plus API review; PIE plus ELF-dependency plus glibc-symbol bounded via existing upstream constraints plus cross-product expansion only after the cohort passes; stop-slice on replacement acquisition engine plus compiler backend plus Cargo graph plus coverage engine with no deferred Rust Windows support plus no premature C/C++ admission plus no weakened hermeticity; Do not implement missing infra merely to fill cross-product; unbounded fork rejected. Backends stay provisional; floors qualified seed-only under issue #500, coverage qualified seed-only under issue #501, corpus qualified seed-only under issue #499, routes qualified seed-only under issue #504; no `Supported` claim. | issue #505 |
