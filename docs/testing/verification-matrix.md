...
qualified seed-only under closed #510; JVM delivered under #796 plus Scala/.NET Layer-2 delivered under #797 plus Native Layer-2 delivered
under #798 plus Structured Protobuf/QML delivered under #799) with taxonomy promotion under #802 (successor to closed #512) and Python future
...
| Go | Delivered (code ownership) | Delivered | Delivered | Delivered (`adopt-go`) | Delivered (contract) | Delivered (`tools/depcheck`) | Delivered | Open | Open |
...
- `prove`: `:target_tags`, `:coverage_cell`,
  ...
  `:skip_budget_qualification` (per-host Linux-only sh skip budget plus
  inventory plus per-cell reporting, issue #769,
  `bazel run //tools/ci:skip_budget_qualification`), `:release_hygiene`,
...
- `dogfood-freshness`: `bazel run //cli/cli:dx -- generate --check //...`,
  ...
  `:supported_evidence_gate`, `:quality_adapters_parity`,
   ...
   under #789), `:docs_pipeline_qualification`,
  ...
  `:scala_dotnet_adapters_qualification` (Scala plus .NET adapter delivery
  pins plus fixture evidence, issue #797),
  `:native_adapters_qualification` (native adapter delivery
  pins plus fixture evidence, issue #798),
  `:structured_adapters_qualification` (Structured adapter delivery
  pins plus fixture evidence, issue #799),
  `:file_family_adapters_qualification` (interpreted/file-family adapter
  delivery pins plus fixture evidence, issue #800),
  ...
  `:windows_eula_qualification` (Windows EULA acknowledgement UX pins
  plus fixture evidence, issue #818; `windows_eula_qualification` 19/19),
  `:laziness_host_qualification` (per-host laziness plus symlink-privilege pins
  plus fixture evidence, issue #917; `laziness_host_qualification` 18/18),
  ...
  `:bump_chain_qualification` (bump widen plus automatic refresh plus
  major-bump=>migrate hint pins plus fixture evidence, issue #638 plus #931),
  ...
  fixture evidence, issue #772),
    ...
    fixture evidence, closed #588 plus #790 plus #791 plus #792 plus #793 plus #794 plus #795),
  ...
  `:cli_strict_qualification` (strict parsing plus help-output goldens pins plus
  fixture evidence, issue #810),
  `:audit_declared_inputs_qualification` (audit curator declared-inputs pins plus
  fixture evidence, issue #812),
  ...
  wont-fix pins plus fixture evidence, issue #645),
   ...
   `:release_packaging_qualification` (artifact-into-releases packaging plus
   SBOM/provenance linkage pins plus fixture evidence, issue #813;
   `release_packaging_qualification` 20/20),
   `:release_matrix_qualification` (release-matrix follow-up cells qualified
   with per-host evidence pins plus fixture evidence, issue #815;
   `release_matrix_qualification` 20/20),
...
`helper_qualification` 27/27, `clap_tokenizer_qualification` 19/19, `cli_strict_qualification` 19/19, `audit_declared_inputs_qualification` 17/17, `release_packaging_qualification` 20/20, `release_matrix_qualification` 20/20,
...
`windows_qualification` 13/13, `windows_acquisition_qualification` 14/14, `acquisition_rights_qualification` 15/15, `windows_eula_qualification` 19/19, `laziness_host_qualification` 18/18, `windows_transport_qualification` 16/16, `prebuilt_interop_qualification` 16/16, `linux_corpus_qualification` 16/16, `lcov_accounting_qualification` 16/16, `cargo_metadata_qualification` 16/16, `strict_generation_qualification` 16/16, `cross_routes_qualification` 17/17, `remediation_bounds_qualification` 16/16, `layer2_opens_qualification` 16/16, `quality_taxonomy_qualification` 25/25, `python_audit_qualification` 24/24, `selective_update_qualification` 16/16, `selective_cargo_qualification` 16/16, `selective_nuget_qualification` 16/16, `selective_maven_qualification` 16/16, `selective_go_qualification` 16/16, `bump_chain_qualification` 20/20, `bump_discovery_qualification` 16/16, `bump_gha_qualification` 16/16, `runner_rotation_qualification` 16/16, `ghcr_rebuild_rotation_qualification` 16/16, `devcontainer_boot_qualification` 16/16, `update_events_qualification` 16/16, `update_rollback_qualification` 16/16, `env_plugins_cgo_qualification` 24/24, `starlark_futures_qualification` 40/40,
