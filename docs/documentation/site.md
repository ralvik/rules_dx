...
execution open (issue #581, live successor to closed #421). Accepted: the `dx_docs` site-build action planning over the
...
is open under issue #581. mdBook is the decided
...
[Testing](../testing/) before any support statement. Open under issue #581
...
at the pre-render boundary remains a gap (issue #581). Neither mode compares against committed IR. Build and check
