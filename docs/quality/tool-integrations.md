...
- **Scala + .NET cohort (#797, successor to closed #417):** adapters `scalafmt` (format `scala`),
  ...
  (per-TFM/RID aggregation decided under issue #492 with `csharp/tests/fixtures/roslyn/` evidence: concatenate per-pivot runs into one SARIF log with a single schema plus version in deterministic pivot order (ascending TFM version, then RID, then   configuration over `automationDetails.id` provenance), union of results with per-pivot provenance; single-SARIF assumption is rejected and merged-single-run is rejected, collection is declared inputs with fail-closed, recorded here, not silent); FSharpLint console-parse is rejected and wire via `FSharpLint.Application.Lint` is required, decided under issue #493 with `fsharp/tests/fixtures/fsharplint/` evidence (standard console blocks carry start-only locations with rule IDs inside the URL plus snippet plus caret plus separator with shared-stream info lines; `-f msbuild` lines carry full ranges with rule IDs but drop fix plus typecheck context; target-coupled `.fsproj`/`.sln` plus `fsharplint.json` from the authoritative target with `ReceivedWarning` callback plus sandbox-apply-and-diff and declared outputs, never `IN_PLACE`), recorded here, not silent; Scalafix has no machine-readable CLI output upstream,
...
Rust formatting is crate-contextual (delivered under issue #468):
...
replacement out), `matrix_rust_format_edition_2015` (2015-only bytes clean
under edition 2015), `matrix_rust_format_edition_mismatch` (same bytes
surface syntax errors under edition 2021), and `dx format --check //rust/...` clean.
