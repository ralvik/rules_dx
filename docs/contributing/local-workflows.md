...
Fresh clone to green build, copy-paste. Linux x86_64 seed host plus Linux arm64 native plus macOS arm64 native plus Windows x86_64 MSVC-compatible native.
...
First-hour evidence is one-shot per ADR 0022:
see First-Hour Timing for the measured
...
Build, test, and coverage run through Bazel on Linux x86_64 (plus Linux arm64 native, plus macOS arm64 native on macos-14, plus Windows x86_64 MSVC-compatible native on windows-latest with shell bash (macOS x86_64 Not planned per #976)) with local-only
execution:
...
(see Code style); `.editorconfig` mirrors the shell
...
type per package, shared `tags = ["corpus"]`) is checked with the
...
same way; see ADR 0021. Coverage
...
(delivered) as the sole updater (native-only):
...
full verification) per the automation policy. Each iteration
...
project coverage gate must pass; the
