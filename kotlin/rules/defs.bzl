...
Each `dx_kotlin_*` macro creates one private `<name>_dx_upstream` target with
...
def _dx_kotlin_quality_sources(ctx):
...
def _dx_kotlin_preserved_library_providers(ctx):
    upstream = ctx.attr.upstream
    if JavaInfo not in upstream:
        fail("dx_kotlin_*: upstream target has no JavaInfo: " + str(ctx.attr.upstream.label))
    return [upstream[JavaInfo]]

def _dx_kotlin_forwarded_output_providers(ctx):
...
def _dx_kotlin_forwarded_java_info(ctx):
...
def _dx_kotlin_forwarded_instrumented(ctx):
...
def _dx_kotlin_library_forward_impl(ctx):
    upstream = ctx.attr.upstream
    if InstrumentedFilesInfo not in upstream:
        fail("dx_kotlin_*: upstream target has no InstrumentedFilesInfo: " + str(ctx.attr.upstream.label))
    return (
        _dx_kotlin_preserved_library_providers(ctx) +
        [upstream[DefaultInfo]] +
        [upstream[InstrumentedFilesInfo]] +
        _dx_kotlin_forwarded_output_providers(ctx) +
        [_dx_kotlin_quality_sources(ctx)]
    )

_dx_kotlin_library_forward = rule(
    implementation = _dx_kotlin_library_forward_impl,
...
def _dx_kotlin_symlink_default_info(ctx):
    upstream = ctx.attr.upstream[DefaultInfo]
    exe = upstream.files_to_run.executable
    if exe == None:
        fail("dx_kotlin_*: upstream target has no executable: " + str(ctx.attr.upstream.label))
...
def _dx_kotlin_binary_forward_impl(ctx):
    return (
        [_dx_kotlin_symlink_default_info(ctx)] +
        _dx_kotlin_forwarded_java_info(ctx) +
        _dx_kotlin_forwarded_instrumented(ctx) +
        _dx_kotlin_forwarded_output_providers(ctx) +
        [_dx_kotlin_quality_sources(ctx)]
    )

_dx_kotlin_binary_forward = rule(
    implementation = _dx_kotlin_binary_forward_impl,
    executable = True,
    ...
    doc = "Executable forwarder for dx_kotlin_binary: symlinks the upstream binary.",
)

def _dx_kotlin_test_forward_impl(ctx):
    upstream = ctx.attr.upstream
    if InstrumentedFilesInfo not in upstream:
        fail("dx_kotlin_*: upstream target has no InstrumentedFilesInfo: " + str(ctx.attr.upstream.label))
    return (
...
_dx_kotlin_forward_test = rule(
    implementation = _dx_kotlin_test_forward_impl,
    test = True,
    provides = _DX_KOTLIN_EXEC_PROVIDES,
    attrs = {
        "srcs": attr.label_list(
    ...
    doc = "Test forwarder for dx_kotlin_test: symlinks the upstream test executable.",
)

def _dx_kotlin_wrap_library(name, srcs, visibility = None, **kwargs):
    _kt_jvm_library(
...
def _dx_kotlin_wrap_binary(name, srcs, visibility = None, **kwargs):
...
def dx_kotlin_library(name, srcs, visibility = None, **kwargs):
    ...
    Args:
      ...
      **kwargs: extra attributes forwarded to the upstream kt_jvm_library
        (deps, resources, data).
    """
    _dx_kotlin_wrap_library(name, srcs, visibility = visibility, **kwargs)

def dx_kotlin_binary(name, srcs = None, main_class = None, visibility = None, **kwargs):
    ...
    _dx_kotlin_wrap_binary(name, effective_srcs, visibility = visibility, **upstream_kwargs)

def dx_kotlin_test(name, srcs, visibility = None, **kwargs):
