...
# - CI surface: ci.yml carries no sbom-arm64 job, no upload-artifact, and no
#   per-host cache prefixes (linux_arm64 sbom-provenance stays an owner-gated
#   local release record in docs/deploy/release-runbook.md; CI never signs
#   PR code and publishes nothing, fork-safe);
# - promotion-checklist cells for this host: Platform-qualified arm64 (#410),
#   dogfood consumer covers linux_arm64 with all nine checks (#408), tag
#   hygiene at 0.0.0 with no v* tags, --verify-tag versioning, arm64
#   sbom-provenance (#803);
...
# CI runs no sbom-arm64 build or verify: no release-tool targets in ci.yml
# (arm64 SBOM stays an owner-gated local release record, not push/PR).
if ! grep -q -F -e 'sbom-arm64' "$ci" &&
  ! grep -q -F -e 'sbom_demo' "$ci" &&
  ! grep -q -F -e 'dx_release_tools_test' "$ci"; then
  ok
else
  bad "ci.yml still carries an sbom-arm64 build plus verify job (want no sbom-arm64 job and no sbom_demo/dx_release_tools_test in CI; SBOM stays owner-gated local release, #803)"
fi

# CI stages no per-host sbom directory and uploads no provenance artifact.
if ! grep -q -F -e 'RUNNER_TEMP/sbom-arm64' "$ci" &&
  ...
  bad "ci.yml still stages or uploads sbom-provenance-linux_arm64 (want no RUNNER_TEMP/sbom-arm64 stage and no upload-artifact; CI publishes nothing, #803)"
fi
...
if ! grep -q -F -e 'actions/upload-artifact' "$ci" &&
  ...
  bad "ci.yml lost its no-upload plus least-privilege setup-bazel record or runbook lost publishes-nothing plus CI-never-signs (want no upload-artifact in CI for sbom-arm64, #803)"
fi
...
if ! grep -q -F -e 'name: sbom' "$ci" &&
  ...
  bad "ci.yml still carries sbom jobs or per-host cache prefixes (want none: all sbom jobs deleted, caching is setup-bazel plus BuildBuddy, #803)"
fi
...
if grep -q -F -e 'name = "signing_demo"' "$release_build" &&
  ...
  bad "release BUILD or runbook lost the owner-gated signing_demo attestation record with ci.yml never signing (#803)"
fi
...
# Per-cell consumer evidence: the dogfood self-call covers linux_arm64 with
...
if grep -q -F -e "platforms: '[\"linux_x86_64\", \"linux_arm64\", \"macos_arm64\", \"windows_x86_64\"]'" "$ci" &&
  ...
  bad "ci.yml lost its dogfood linux_arm64 all-nine-checks per-cell evidence (want four-platform self-call plus min_coverage 97 plus no disabled_checks, #803)"
fi
...
if grep -q -F -e 'SPDX-2.3 via //deploy/release:sbom_demo' "$pins" &&
  ...
  grep -q -F -e 'ci.yml carries no sbom-arm64 job; SBOM stays a local target under #803' "$pins" &&
  grep -q -F -e 'no upload-artifact, no RUNNER_TEMP stage; CI publishes nothing under #803' "$pins" &&
  ...
  grep -q -F -e 'dogfood consumer self-call covers linux_arm64 with full dx test plus dx coverage' "$pins" &&
  ...
  bad "pins.bzl lost its wire plus no-CI-upload plus cells plus attestation plus rejected plus honesty pins under issue #803"
fi
...
if grep -q -F -e 'Release evidence for Linux arm64 glibc (issue #803)' "$expected" &&
  ...
  grep -q -F -e 'ci.yml carries no sbom-arm64 job' "$expected" &&
  ...
  bad "release_arm64.expected lost its no-CI-upload plus cells plus rejected plus honesty lines under #803"
fi
