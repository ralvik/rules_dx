...
if [[ "$preset_flag_count" == "$fragment_flag_count" ]] && [[ "$preset_flag_count" == "13" ]]; then
  ok
else
  bad "preset flag drift (lib.rs has $preset_flag_count, preset.bazelrc has $fragment_flag_count; want 13 each: run preset.update)"
fi
stale_preset=0
