...
if grep -q -F -e 'name = "windows_acquisition_qualification"' "tools/ci/ci_targets_d.bzl" &&
  grep -q -F -e 'bazel run --noshow_progress //tools/ci:windows_acquisition_qualification' tools/ci/dogfood_freshness.sh; then
  ok
else
  bad "tools/ci/ci_targets_d.bzl or dogfood_freshness.sh lost the windows_acquisition_qualification wiring (want target plus dogfood-freshness)"
fi
