...
if grep -q -F -e 'ADAPTER_LESS_GO = "go"' "$pins" &&
  ...
  grep -q -F -e 'ADAPTER_LESS_SCALA = "scala"' "$pins" &&
  grep -q -F -e 'ADAPTER_LESS_CSHARP = "csharp"' "$pins" &&
  grep -q -F -e 'ADAPTER_LESS_FSHARP = "fsharp"' "$pins" &&
...
opens_matrix=""
for cell in matrix_go_ matrix_java_ matrix_kotlin_ matrix_scala_ matrix_csharp_ matrix_fsharp_ matrix_c_ matrix_cpp_ matrix_vue_ matrix_svelte_ matrix_astro_ matrix_mdx_; do
...
opens_deferred=""
for cls in go c cpp java kotlin scala csharp fsharp vue svelte astro mdx; do
