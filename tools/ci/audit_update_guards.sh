#!/usr/bin/env bash
# Audit/update/depcheck execution guards (; opens under).
#
# Live `dx audit` executes qualified auditors per family over resolved scopes
# with per-family reporting
...
# delivered in tools/depcheck/ (; remaining opens under); remaining adapter
...
# live audit executes qualified auditors with per-family reporting (no deferred code).
...
# aggregate exit-code mapping stays unit-pinned (clean 0, findings
...
# aggregate exit-code mapping stays unit-pinned (overall_failure).
...
# plus: depcheck contract harness stays green (truth table + delivered routes).
