...
if grep -q -F -e 'nuget:FSharp.Core' "$exec_update" &&
  grep -q -F -e 'live_unsupported_nuget_selective_fails_without_launch' "$exec_update" &&
...
if grep -q -F -e 'name = "selective_nuget_qualification"' "$build" &&
  grep -q -F -e 'selective_nuget_qualification.sh' "$build" &&
  grep -q -F -e 'bazel run --noshow_progress //tools/ci:selective_nuget_qualification' "$ci"; then
  ok
else
