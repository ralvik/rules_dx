...
#   both Cargo-flavor semver, major-release-only gate
#   (`dx_adopt::migrate_is_major_bump` plus `plan_migrate`), prerelease
...
#   `dx_adopt::migrate_manifest_name`), edit-manifest records applied
...
# Planning library owns the major-release-only gate plus manifest selection.
...
# Manifest selection stays mechanical per major hop.
if grep -q -F -e 'migrate-v{from_major}-to-v{to_major}.json' "$migrate_rs" &&
  grep -q -F -e 'migrate-v1-to-v2.json' "$migrate_rs" &&
  grep -q -F -e 'migrate-v0-to-v1.json' "$migrate_rs"; then
  ok
else
  bad "migrate manifest selection lost its per-major-hop pin"
fi
...
if grep -q -F -e 'delivered CLI' "$migrate_doc" &&
  ...
  grep -q -F -e 'issue #462' "$migrate_doc"; then
  ok
else
...
if grep -q -F -e '`migrate`' "$testing" &&
  ...
  grep -q -F -e 'issue #462' "$testing"; then
  ok
else
  bad "testing/cli.md lost its migrate fixture plus #462 owner record"
fi
