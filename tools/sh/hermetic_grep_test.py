...
class HermeticGrepTest(unittest.TestCase):
    ...
    def test_contains_fixed(self):
        ...
        self.assertEqual(run("contains", path, "--fixed", "--", "extra_target_triples").returncode, 0)
        self.assertEqual(run("contains", path, "--fixed", "--", "missing-lit").returncode, 1)

    def test_contains_all_patterns(self):
        path = self.write("b.txt", "alpha\nbeta\ngamma\n")
        self.assertEqual(run("contains", path, "--fixed", "--", "alpha", "beta").returncode, 0)
        self.assertEqual(run("contains", path, "--fixed", "--", "alpha", "nope").returncode, 1)

    def test_contains_re(self):
        path = self.write("c.txt", "version = \"1.26.6\"\n")
        self.assertEqual(run("contains", path, "--re", "--", r'version = "[^"]+"').returncode, 0)
        self.assertEqual(run("contains", path, "--re", "--", r"no-match-\d+").returncode, 1)

    def test_absent(self):
        path = self.write("d.txt", "clean file\n")
        self.assertEqual(run("absent", path, "--fixed", "--", "forbidden").returncode, 0)
        self.assertEqual(run("absent", path, "--fixed", "--", "clean").returncode, 1)

    def test_missing_file_fails_closed(self):
        missing = os.path.join(self.tmp.name, "nope.txt")
        self.assertNotEqual(run("contains", missing, "--fixed", "--", "x").returncode, 0)
    ...
    def test_tree_contains_include(self):
        ...
        proc = run("tree-contains", "--fixed", "--include", "*.yml", "--roots", self.tmp.name, "--", "runs-on: windows-latest")
        self.assertEqual(proc.returncode, 0)
        proc = run("tree-contains", "--fixed", "--include", "*.txt", "--roots", self.tmp.name, "--", "runs-on: windows-latest")
    ...
    def test_tree_list(self):
        ...
        proc = run("tree-list", "--fixed", "--include", "BUILD.bazel", "--roots", self.tmp.name, "--", "gazelle_binary")
        self.assertEqual(proc.returncode, 0)
    ...
    def test_context_contains_fixed(self):
        ...
        proc = run("context-contains", path, "build-windows-x86_64", "-A", "30", "--anchor-fixed", "--fixed", "--", "secrets.")
        self.assertEqual(proc.returncode, 0)
        proc = run("context-contains", path, "build-windows-x86_64", "-A", "1", "--anchor-fixed", "--fixed", "--", "secrets.")
        self.assertEqual(proc.returncode, 1)

    def test_context_anchor_re(self):
        path = self.write("ci2.yml", "  test:\n    needs: [build]\n")
        proc = run("context-contains", path, r"^  test:", "-A", "3", "--anchor-re", "--fixed", "--", "needs: [build]")
        self.assertEqual(proc.returncode, 0)

    def test_context_absent(self):
        path = self.write("ci3.yml", "build-macos-arm64\nline1\nclean\n")
        proc = run("context-absent", path, "build-macos-arm64", "-A", "20", "--anchor-fixed", "--re", "--", r"secrets\.|GH_TOKEN")
        self.assertEqual(proc.returncode, 0)

    def test_extract_quoted(self):
        path = self.write("codegen.bzl", 'DX_CODEGEN_PLAN_OUTPUT_GROUP = "dx_codegen_plans"\n')
    ...
    def test_tree_count(self):
        ...
        proc = run("tree-count", "--fixed", "--include", "*.yml", "--roots", self.tmp.name, "--", "pin")
        self.assertEqual(proc.returncode, 0)
