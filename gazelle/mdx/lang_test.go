...
func TestGenerateFailures(t *testing.T) {
	cases := []struct {
		...
		{"readError", map[string]string{}, []string{"missing.mdx"}, "read missing.mdx"},
		{"emptyName", map[string]string{"pkg/demo/---.mdx": mdxDoc("export const x = 1;\n")}, []string{"---.mdx"}, "empty target name"},
		{"collision", map[string]string{
			"pkg/demo/a-b.mdx": mdxDoc("export const x = 1;\n"),
			"pkg/demo/a_b.mdx": mdxDoc("export const x = 1;\n"),
		}, []string{"a-b.mdx", "a_b.mdx"}, "claimed by a-b.mdx, a_b.mdx"},
	}
...
func TestCheckClaims(t *testing.T) {
	claimants := []Claimant{{Name: "demo", Source: "demo.mdx", Kind: libraryKind}}
	...
	dupes := []Claimant{{Name: "a_b", Source: "a-b.mdx", Kind: libraryKind}, {Name: "a_b", Source: "a_b.mdx", Kind: libraryKind}}
...
func TestImportsIndexesLibraries(t *testing.T) {
	...
	lib.SetAttr("srcs", []string{"demo.mdx", "notes.txt"})
...
func resolverIndex(lang *mdxLang, entries ...struct {
	pkg  string
	name string
	ext  string
}) *resolve.RuleIndex {
	index := resolve.NewRuleIndex(func(r *rule.Rule, _ string) resolve.Resolver {
		if _, ok := mdxKinds[r.Kind()]; ok {
			return lang
		}
		return nil
	})
...
func TestResolveOverride(t *testing.T) {
	l := &mdxLang{}
	cfg := resolverConfig(t, []rule.Directive{{Key: "resolve", Value: "mdx mdx mapped //mapped:dep"}})
	r := rule.NewRule(libraryKind, "app")
...
func TestIgnoreDirectiveInheritanceAndStaleCheck(t *testing.T) {
	...
	l.Before(context.Background())
	root := config.New()
	file, err := rule.LoadData("BUILD.bazel", "", []byte("# gazelle:dx_ignore_import mdx missing_dep\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestStaleIgnoreFails(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := config.New()
	file, err := rule.LoadData("BUILD.bazel", "pkg", []byte("# gazelle:dx_ignore_import mdx stale\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveIgnore(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := resolverConfig(t, nil)
	file, err := rule.LoadData("BUILD.bazel", "app", []byte("# gazelle:dx_ignore_import mdx external\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveMappingIgnoreConflict(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := resolverConfig(t, []rule.Directive{{Key: "resolve", Value: "mdx mdx mapped //mapped:dep"}})
	file, err := rule.LoadData("BUILD.bazel", "app", []byte("# gazelle:dx_ignore_import mdx mapped\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "app", file)
...
func TestConfigureSkipsOtherDirectives(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := config.New()
	file, err := rule.LoadData("BUILD.bazel", "pkg", []byte("# gazelle:resolve mdx mdx foo //foo:bar\n# gazelle:dx_ignore_import python foo\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "pkg", file)
...
func TestConfigureThreeFieldAndMalformed(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := config.New()
	file, err := rule.LoadData("BUILD.bazel", "pkg", []byte("# gazelle:dx_ignore_import mdx mdx mydep\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "pkg", file)
	...
	badCfg := config.New()
	badFile, err := rule.LoadData("BUILD.bazel", "pkg", []byte("# gazelle:dx_ignore_import mdx\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestMatchingIgnoreMiss(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := config.New()
	file, err := rule.LoadData("BUILD.bazel", "app", []byte("# gazelle:dx_ignore_import mdx other\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "app", file)
