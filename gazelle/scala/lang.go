...
// NewLanguage returns the private first-party Scala Gazelle extension.
...
// Imports indexes one reusable import identity per Scala source owned by a
...
func (l *scalaLang) generateRules(args language.GenerateArgs) language.GenerateResult {
	...
	for _, src := range sources {
		...
		if err != nil {
			l.fail("scala: %s: read %s: %v", args.Rel, src, err)
			continue
		}
		...
		if err != nil {
			l.fail("scala: %s: parse package %s: %v", args.Rel, src, err)
			continue
		}
		packages[pkg] = true
	...
	if len(packages) > 1 {
		...
		sort.Strings(names)
		l.fail("scala: %s: mixed packages %s in one directory; split the directory before adopting generation", args.Rel, strings.Join(names, ", "))
		return language.GenerateResult{}
	}
...
func mergeStale(file *rule.File, result language.GenerateResult) language.GenerateResult {
	...
	for _, existing := range file.Rules {
		if _, owned := scalaKinds[existing.Kind()]; !owned || desired[existing.Kind()+"\x00"+existing.Name()] {
			continue
		}
...
func (l *scalaLang) Resolve(c *config.Config, ix *resolve.RuleIndex, _ *repo.RemoteCache, r *rule.Rule, raw interface{}, from label.Label) {
	...
	for _, name := range imports.imports {
		...
		default:
			l.fail("scala: %s: ambiguous import %q resolves to %s", from, name, formatMatches(matches))
		}
	}
	if len(deps) == 0 {
		return
