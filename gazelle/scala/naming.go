...
// holds one reusable `scala_library` named after the directory basename,
// with `srcs` as the sorted non-test `.scala` files. `*Test.scala` files are
// never library sources (handwritten `scala_test` owns them), thin
// `scala_binary` entries are never inferred, and directories mixing a
