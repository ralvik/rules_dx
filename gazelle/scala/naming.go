// Naming implements the deterministic Scala target-name normalizer owned by
// the first-party Scala Gazelle extension.
//
...
//
// Scala is package-level (Go-style, not Python one-source): one directory
// holds one reusable `dx_scala_library` named after the directory basename,
// with `srcs` as the sorted non-test `.scala` files. `*Test.scala` files are
// never library sources (handwritten `dx_scala_test` owns them), thin
// `dx_scala_binary` entries are never inferred, and directories mixing a
...
// SupportedExts are the Scala source extensions discovered by the extension.
// Only `.scala` is listed; `*Test.scala` files are discovered then excluded
...
const LibraryKind = "dx_scala_library"
...
// IsTestSource reports whether a `.scala` basename is test-owned: the stem
...
// TargetName derives the Bazel target name for one Scala source path: the
...
// ClassIdentity returns the import identity for one Scala import or owned
