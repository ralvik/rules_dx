...
// NewLanguage returns the private first-party Ruby Gazelle extension.
...
// Imports indexes one reusable import identity per Ruby source owned by a
...
func (l *rubyLang) generateRules(args language.GenerateArgs) language.GenerateResult {
	...
	for _, src := range sources {
		...
		if err != nil {
			l.fail("ruby: %s: read %s: %v", args.Rel, src, err)
			continue
		}
		if DefinesMain(content) {
			l.fail("ruby: %s: %s defines main; thin ruby_binary entries stay handwritten, so split main-bearing sources into their own directory before adopting generation", args.Rel, src)
			continue
		}
		pkg, err := ParsePackage(content)
		if err != nil {
			l.fail("ruby: %s: parse package %s: %v", args.Rel, src, err)
			continue
		}
		packages[pkg] = true
	...
	if len(packages) > 1 {
		...
		sort.Strings(names)
		l.fail("ruby: %s: mixed packages %s in one directory; split the directory before adopting generation", args.Rel, strings.Join(names, ", "))
		return language.GenerateResult{}
	}
...
func mergeStale(file *rule.File, result language.GenerateResult) language.GenerateResult {
	...
	for _, existing := range file.Rules {
		if _, owned := rubyKinds[existing.Kind()]; !owned || desired[existing.Kind()+"\x00"+existing.Name()] {
			continue
		}
...
func (l *rubyLang) Resolve(c *config.Config, ix *resolve.RuleIndex, _ *repo.RemoteCache, r *rule.Rule, raw interface{}, from label.Label) {
	...
	for _, name := range imports.imports {
		...
		default:
			l.fail("ruby: %s: ambiguous import %q resolves to %s", from, name, formatMatches(matches))
		}
	}
	if len(deps) == 0 {
		return
