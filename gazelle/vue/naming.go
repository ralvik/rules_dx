// Naming implements the deterministic Vue target-name normalizer owned
// by the first-party Vue Gazelle extension.
//
...
//
// Every supported `.vue` single-file component receives one ordinary
// reusable one-source `vue_library`. Vue tests and entries stay
// JavaScript-owned (parsed or compiled outputs exercised through the
// `javascript_test`/`javascript_binary` wrappers); `.vue` sources are never tests
...
// SupportedExts are the Vue source extensions discovered by the
...
// stripVueExt reports the basename without its final Vue extension.
...
// TargetName derives the Bazel target name for one Vue source path:
...
// ModuleName returns the import identity for one Vue source path:
