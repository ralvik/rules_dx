...
// generated rule's script block. Standard-library roots are dropped at
// collection; every other root resolves strictly or fails generation.
type targetImports struct {
	imports []string
}

// NewLanguage returns the private first-party Vue Gazelle extension.
...
// Imports indexes one reusable import identity per Vue source owned
...
func (l *vueLang) generateRules(args language.GenerateArgs) language.GenerateResult {
	...
	for _, src := range sources {
		...
		if err != nil {
			l.fail("vue: %s: read %s: %v", args.Rel, src, err)
			continue
		}
		name, err := TargetName(src)
		if err != nil {
			l.fail("vue: %s: %v", args.Rel, err)
			continue
		}
...
func mergeStale(file *rule.File, result language.GenerateResult) language.GenerateResult {
	...
	for _, existing := range file.Rules {
		if _, owned := vueKinds[existing.Kind()]; !owned || desired[existing.Kind()+"\x00"+existing.Name()] {
			continue
		}
...
func (l *vueLang) Resolve(c *config.Config, ix *resolve.RuleIndex, _ *repo.RemoteCache, r *rule.Rule, raw interface{}, from label.Label) {
	...
	for _, name := range imports.imports {
		...
		default:
			l.fail("vue: %s: ambiguous import %q resolves to %s", from, name, formatMatches(matches))
		}
	}
	if len(deps) == 0 {
		return
