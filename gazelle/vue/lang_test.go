...
func TestGenerateFailures(t *testing.T) {
	cases := []struct {
		...
		{"readError", map[string]string{}, []string{"missing.vue"}, "read missing.vue"},
		{"emptyName", map[string]string{"pkg/demo/---.vue": sfc("export default {};\n")}, []string{"---.vue"}, "empty target name"},
		{"collision", map[string]string{
			"pkg/demo/a-b.vue": sfc("export default {};\n"),
			"pkg/demo/a_b.vue": sfc("export default {};\n"),
		}, []string{"a-b.vue", "a_b.vue"}, "claimed by a-b.vue, a_b.vue"},
	}
...
func TestCheckClaims(t *testing.T) {
	claimants := []Claimant{{Name: "demo", Source: "demo.vue", Kind: libraryKind}}
	...
	dupes := []Claimant{{Name: "a_b", Source: "a-b.vue", Kind: libraryKind}, {Name: "a_b", Source: "a_b.vue", Kind: libraryKind}}
...
func TestImportsIndexesLibraries(t *testing.T) {
	...
	lib.SetAttr("srcs", []string{"demo.vue", "notes.txt"})
...
func resolverIndex(lang *vueLang, entries ...struct {
	pkg  string
	name string
	ext  string
}) *resolve.RuleIndex {
	index := resolve.NewRuleIndex(func(r *rule.Rule, _ string) resolve.Resolver {
		if _, ok := vueKinds[r.Kind()]; ok {
			return lang
		}
		return nil
	})
...
func TestResolveBranches(t *testing.T) {
	...
	index := resolverIndex(l,
		struct{ pkg, name string; ext string }{"lib/b", "b", ".vue"},
		struct{ pkg, name string; ext string }{"lib/a", "a", ".vue"},
	)
...
func TestResolveOverride(t *testing.T) {
	l := &vueLang{}
	cfg := resolverConfig(t, []rule.Directive{{Key: "resolve", Value: "vue vue mapped //mapped:dep"}})
	r := rule.NewRule(libraryKind, "app")
...
func TestResolveAmbiguous(t *testing.T) {
	...
	cfg := resolverConfig(t, nil)
	index := resolverIndex(l,
		struct{ pkg, name string; ext string }{"one", "same", ".vue"},
		struct{ pkg, name string; ext string }{"two", "same", ".vue"},
	)
...
func TestIgnoreDirectiveInheritanceAndStaleCheck(t *testing.T) {
	...
	l.Before(context.Background())
	root := config.New()
	file, err := rule.LoadData("BUILD.bazel", "", []byte("# gazelle:dx_ignore_import vue missing_dep\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestStaleIgnoreFails(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := config.New()
	file, err := rule.LoadData("BUILD.bazel", "pkg", []byte("# gazelle:dx_ignore_import vue stale\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveIgnore(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := resolverConfig(t, nil)
	file, err := rule.LoadData("BUILD.bazel", "app", []byte("# gazelle:dx_ignore_import vue external\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveMappingIgnoreConflict(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := resolverConfig(t, []rule.Directive{{Key: "resolve", Value: "vue vue mapped //mapped:dep"}})
	file, err := rule.LoadData("BUILD.bazel", "app", []byte("# gazelle:dx_ignore_import vue mapped\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "app", file)
...
func TestConfigureSkipsOtherDirectives(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := config.New()
	file, err := rule.LoadData("BUILD.bazel", "pkg", []byte("# gazelle:resolve vue vue foo //foo:bar\n# gazelle:dx_ignore_import python foo\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "pkg", file)
...
func TestConfigureThreeFieldAndMalformed(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := config.New()
	file, err := rule.LoadData("BUILD.bazel", "pkg", []byte("# gazelle:dx_ignore_import vue vue mydep\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "pkg", file)
	...
	badCfg := config.New()
	badFile, err := rule.LoadData("BUILD.bazel", "pkg", []byte("# gazelle:dx_ignore_import vue\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestMatchingIgnoreMiss(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := config.New()
	file, err := rule.LoadData("BUILD.bazel", "app", []byte("# gazelle:dx_ignore_import vue other\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "app", file)
