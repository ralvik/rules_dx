// Naming implements the deterministic FSharp target-name normalizer owned by
// the first-party FSharp Gazelle extension.
//
...
// holds one reusable `dx_fsharp_library` named after the directory basename,
// with `srcs` as the sorted non-test `.fs` files. `*Test.fs` files are
// never library sources (handwritten `dx_fsharp_test` owns them), thin
// `dx_fsharp_binary` entries are never inferred, and directories mixing a
...
// SupportedExts are the FSharp source extensions discovered by the extension.
// Only `.fs` is listed; `*Test.fs` files are discovered then excluded
...
const LibraryKind = "dx_fsharp_library"
...
// IsTestSource reports whether a `.fs` basename is test-owned: the stem
...
// TargetName derives the Bazel target name for one FSharp source path: the
...
// ClassIdentity returns the import identity for one FSharp import or owned
