// Naming implements the deterministic Java target-name normalizer owned by
// the first-party Java Gazelle extension.
//
...
// SupportedExts are the Java source extensions discovered by the extension.
// Only `.java` is listed; `*Test.java` files are discovered then excluded
...
// IsTestSource reports whether a `.java` basename is test-owned: the stem
...
// TargetName derives the Bazel target name for one Java source path: the
...
// ClassIdentity returns the import identity for one Java import or owned
