...
func TestGeneratePackageLevelLibrary(t *testing.T) {
	...
	if lib.Kind() != LibraryKind || lib.Name() != "demo" {
		t.Fatalf("library = %s(%s), want dx_java_library(demo)", lib.Kind(), lib.Name())
	}
...
func TestLanguageMetadata(t *testing.T) {
	l := &javaLang{}
	if l.Name() != "java" || len(l.Kinds()) != 1 || l.CheckFlags(flag.NewFlagSet("test", flag.ContinueOnError), config.New()) != nil {
	...
	if len(loads) != 1 || loads[0].Name != "@renamed_dx//java/rules:defs.bzl" || strings.Join(loads[0].Symbols, ",") != "dx_java_library" {
		t.Errorf("apparent loads = %+v", loads)
	}
	if defaults := l.Loads(); len(defaults) != 1 || defaults[0].Name != "@rules_dx//java/rules:defs.bzl" {
		t.Errorf("default loads = %+v", defaults)
	}
	defaultApparent := l.ApparentLoads(func(string) string { return "" })
	if defaultApparent[0].Name != "@rules_dx//java/rules:defs.bzl" {
...
func resolverIndex(lang *javaLang, entries ...struct {
	pkg  string
	name string
}) *resolve.RuleIndex {
	index := resolve.NewRuleIndex(func(r *rule.Rule, _ string) resolve.Resolver {
		if _, ok := javaKinds[r.Kind()]; ok {
			return lang
		}
		return nil
	})
	for _, entry := range entries {
		...
		r.SetAttr("srcs", []string{entry.name + ".java"})
...
func TestResolveOverride(t *testing.T) {
	l := &javaLang{}
	cfg := resolverConfig(t, []rule.Directive{{Key: "resolve", Value: "java java Mapped //mapped:dep"}})
	r := rule.NewRule(LibraryKind, "app")
...
func TestResolveIgnore(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := resolverConfig(t, nil)
	file, err := rule.LoadData("BUILD.bazel", "app", []byte("# gazelle:dx_ignore_import java Dropped\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveStaleIgnoreFails(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := resolverConfig(t, nil)
	file, err := rule.LoadData("BUILD.bazel", "app", []byte("# gazelle:dx_ignore_import java Unused\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "app", file)
