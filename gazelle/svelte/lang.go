...
// NewLanguage returns the private first-party Svelte Gazelle extension.
func NewLanguage() language.Language { return &svelteLang{} }

func (l *svelteLang) Before(context.Context) { l.errors = nil; l.ignores = nil }

func (l *svelteLang) DoneGeneratingRules() {}

func (l *svelteLang) RegisterFlags(*flag.FlagSet, string, *config.Config) {}

func (l *svelteLang) CheckFlags(*flag.FlagSet, *config.Config) error { return nil }

func (*svelteLang) KnownDirectives() []string { return []string{"dx_ignore_import"} }

func (l *svelteLang) Configure(c *config.Config, rel string, f *rule.File) {
	var inherited []*ignoreEntry
	if raw, ok := c.Exts[languageName]; ok {
		inherited = append(inherited, raw.(*svelteConfig).ignores...)
	}
	if f != nil {
		for _, directive := range f.Directives {
			...
			if len(fields) == 2 && fields[0] == languageName {
				...
				l.fail("svelte: //%s: malformed # gazelle:dx_ignore_import %s", rel, directive.Value)
			}
		}
	}
	c.Exts[languageName] = &svelteConfig{ignores: inherited}
}

func (l *svelteLang) fail(format string, args ...interface{}) {
	l.errors = append(l.errors, fmt.Sprintf(format, args...))
}

func (l *svelteLang) AfterResolvingDeps(context.Context) {
	for _, ignore := range l.ignores {
		if !ignore.used {
			l.fail("svelte: //%s: stale # gazelle:dx_ignore_import svelte %s matches no literal reference", ignore.path, ignore.value)
		}
	}
	if len(l.errors) == 0 {
		return
	...
	panic("Svelte generation failed:\n" + strings.Join(l.errors, "\n"))
}

func (*svelteLang) Name() string { return languageName }

func (*svelteLang) Kinds() map[string]rule.KindInfo { return svelteKinds }

func (*svelteLang) Loads() []rule.LoadInfo {
	return svelteLoads("rules_dx")
}

func (l *svelteLang) ApparentLoads(moduleToApparentName func(string) string) []rule.LoadInfo {
	repoName := moduleToApparentName("rules_dx")
	if repoName == "" {
		repoName = "rules_dx"
	}
	return svelteLoads(repoName)
}

func svelteLoads(rulesRepo string) []rule.LoadInfo {
	return []rule.LoadInfo{
		{Name: "@" + rulesRepo + "//svelte/rules:defs.bzl", Symbols: []string{libraryKind}},
	}
}

// Imports indexes one reusable import identity per Svelte source owned
// by a library rule: the exact module stem.
func (*svelteLang) Imports(_ *config.Config, r *rule.Rule, _ *rule.File) []resolve.ImportSpec {
...
func (*svelteLang) Embeds(*rule.Rule, label.Label) []label.Label { return nil }

func (l *svelteLang) GenerateRules(args language.GenerateArgs) language.GenerateResult {
...
func (l *svelteLang) generateRules(args language.GenerateArgs) language.GenerateResult {
	...
	for _, src := range sources {
		...
		if err != nil {
			l.fail("svelte: %s: read %s: %v", args.Rel, src, err)
			continue
		}
		name, err := TargetName(src)
		if err != nil {
			l.fail("svelte: %s: %v", args.Rel, err)
			continue
		}
...
func mergeStale(file *rule.File, result language.GenerateResult) language.GenerateResult {
	...
	for _, existing := range file.Rules {
		if _, owned := svelteKinds[existing.Kind()]; !owned || desired[existing.Kind()+"\x00"+existing.Name()] {
			continue
		}
...
func (l *svelteLang) Resolve(c *config.Config, ix *resolve.RuleIndex, _ *repo.RemoteCache, r *rule.Rule, raw interface{}, from label.Label) {
	...
	for _, name := range imports.imports {
		...
		if override, found := resolve.FindRuleWithOverride(c, spec, languageName); found {
			if ignore := matchingIgnore(c, name); ignore != nil {
				ignore.used = true
				l.fail("svelte: %s: import %q has both an exact resolve mapping and ignore", from, name)
				continue
			}
		...
		case 0:
			...
			l.fail("svelte: %s: unresolved import %q; add a local one-source library or an exact # gazelle:resolve mapping", from, name)
		default:
			l.fail("svelte: %s: ambiguous import %q resolves to %s", from, name, formatMatches(matches))
		}
	}
	if len(deps) == 0 {
		return
...
func matchingIgnore(c *config.Config, name string) *ignoreEntry {
	...
	for i := len(raw.(*svelteConfig).ignores) - 1; i >= 0; i-- {
		if entry := raw.(*svelteConfig).ignores[i]; entry.value == name {
			return entry
		}
	}
	return nil
