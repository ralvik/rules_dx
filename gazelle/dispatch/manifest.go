...
func (r *manifestRecorder) emit(ignores []collectedIgnore) error {
	...
	if err != nil {
		return fmt.Errorf("dispatch: cannot encode intended manifest: %v", err) // LCOV_EXCL_LINE - policy: docs/testing/README.md#coverage
	}
...
func (r *manifestRecorder) witness(rec packageRecord) (intendedFile, bool, error) {
	...
	if len(file.Edits) == 0 {
		return file, false, fmt.Errorf("dispatch: changed package %q produced no edits", rec.rel) // LCOV_EXCL_LINE - policy: docs/testing/README.md#coverage
	}
	return file, true, nil
}
