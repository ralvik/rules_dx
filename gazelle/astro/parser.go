// Parser extracts the narrow recognized source facts the Astro Gazelle
// extension needs for one-source ownership and strict dependency resolution.
//
// A `.astro` component carries a frontmatter/server-script fence,
// template markup, client `<script>` blocks, and a style block. The
// frontmatter fence and every `<script>` block contribute dependency
// references: both execute as modules, so both resolve identically.
// Template markup and `<style>` regions are inert for dependency
// discovery.
//
// Recognition is by a narrow block parser, never by regular expression:
// splitFence locates the `---` frontmatter fence lines textually, and
...
// Generation never compiles the container. A `---` line inside
// frontmatter string content closes the fence textually; the pinned
...
//
// A component without frontmatter or script blocks, or with an
// unparseable block boundary (an unclosed `---` fence, or a client
// `<script>` element whose boundary never parses), contributes no
// imports and stays inert: it never falls back to a guessed
// JavaScript/TypeScript owner. One unparseable region makes the whole
// component inert, never partially attributed.
//
// Specifier normalization: relative (`./`, `../`, `/`) references contribute
// their basename without the final extension (`./helper.js` -> `helper`);
// bare specifiers contribute the full literal (`astro`,
// `@astrojs/compiler`); `node:`-prefixed and builtin identities are included
...
// Astro component. Only the frontmatter fence and `<script>` blocks are
// examined; template markup and style regions never contribute.
// Standard-library identities are included; callers filter them via
// IsStdLib.
func ParseImports(content []byte) []string {
	...
	if scripts == nil && opensScript(body) {
		// Unparseable client-script boundary: whole component inert.
		return nil
	}
...
// splitFence locates the frontmatter fence of an Astro component. It
// reports fenced=true when the first line is a `---` fence opener
// (trailing spaces, tabs, and carriage returns tolerated). front is the
// raw server-script content between the fences and rest is the component
// remainder after the closing fence line. ok=false reports an opening
// fence with no closing fence line: an unparseable boundary that makes
// the whole component inert. Only a file-start fence counts; a `---`
// line later in the file is template content, never a fence.
...
// trimFenceLine strips the trailing blank characters tolerated on a
// fence line: spaces, tabs, and carriage returns. Leading whitespace is
// significant: an indented `---` is content, never a fence.
func trimFenceLine(line []byte) []byte {
	return bytes.TrimRight(line, " \t\r")
}

// opensScript reports whether body attempts to open a `<script>` element
// outside HTML comments: the tag name matches case-insensitively (the
// scanner consumes the full name, so `scriptx` never matches), even when
// the open tag never closes. ParseImports uses it to tell "no client
// scripts" apart from "unparseable script boundary": only the latter
// makes the component inert.
...
// element in a client-script body (the component remainder after the
// frontmatter fence), in source order, or nil when there is none or a
// block boundary is unparseable. Matching is case-insensitive for the
// tag name; the scanner consumes the full tag name, so `<scriptx>` never
// matches. HTML comments (`<!--` ... `-->`) are skipped, and quoted
...
// result nil, never partially attributed.
