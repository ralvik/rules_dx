...
// library by the Astro Gazelle extension (M20, O41). Astro frontmatter
// and client `<script>` blocks execute as JavaScript/TypeScript modules,
...
// bare (`fs`) and `node:`-prefixed (`node:fs`) forms are recognized; the
// list follows the Node release backing the pinned aspect_rules_js
// toolchain.
package astro
