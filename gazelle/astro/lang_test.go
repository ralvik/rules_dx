...
func TestGenerateFailures(t *testing.T) {
	cases := []struct {
		...
		{"readError", map[string]string{}, []string{"missing.astro"}, "read missing.astro"},
		{"emptyName", map[string]string{"pkg/demo/---.astro": component("export const x = 1;\n")}, []string{"---.astro"}, "empty target name"},
		{"collision", map[string]string{
			"pkg/demo/a-b.astro": component("export const x = 1;\n"),
			"pkg/demo/a_b.astro": component("export const x = 1;\n"),
		}, []string{"a-b.astro", "a_b.astro"}, "claimed by a-b.astro, a_b.astro"},
	}
...
func TestCheckClaims(t *testing.T) {
	claimants := []Claimant{{Name: "demo", Source: "demo.astro", Kind: libraryKind}}
	...
	dupes := []Claimant{{Name: "a_b", Source: "a-b.astro", Kind: libraryKind}, {Name: "a_b", Source: "a_b.astro", Kind: libraryKind}}
...
func TestImportsIndexesLibraries(t *testing.T) {
	...
	lib.SetAttr("srcs", []string{"demo.astro", "notes.txt"})
...
func TestLanguageMetadata(t *testing.T) {
	...
	if len(loads) != 1 || loads[0].Name != "@renamed_dx//astro/rules:defs.bzl" || strings.Join(loads[0].Symbols, ",") != "dx_astro_library" {
...
func resolverIndex(lang *astroLang, entries ...struct {
	pkg  string
	name string
	ext  string
}) *resolve.RuleIndex {
	index := resolve.NewRuleIndex(func(r *rule.Rule, _ string) resolve.Resolver {
		if _, ok := astroKinds[r.Kind()]; ok {
			return lang
		}
		return nil
	})
...
func TestResolveOverride(t *testing.T) {
	l := &astroLang{}
	cfg := resolverConfig(t, []rule.Directive{{Key: "resolve", Value: "astro astro mapped //mapped:dep"}})
	r := rule.NewRule(libraryKind, "app")
...
func TestIgnoreDirectiveInheritanceAndStaleCheck(t *testing.T) {
	...
	l.Before(context.Background())
	root := config.New()
	file, err := rule.LoadData("BUILD.bazel", "", []byte("# gazelle:dx_ignore_import astro missing_dep\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestStaleIgnoreFails(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := config.New()
	file, err := rule.LoadData("BUILD.bazel", "pkg", []byte("# gazelle:dx_ignore_import astro stale\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveIgnore(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := resolverConfig(t, nil)
	file, err := rule.LoadData("BUILD.bazel", "app", []byte("# gazelle:dx_ignore_import astro external\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveMappingIgnoreConflict(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := resolverConfig(t, []rule.Directive{{Key: "resolve", Value: "astro astro mapped //mapped:dep"}})
	file, err := rule.LoadData("BUILD.bazel", "app", []byte("# gazelle:dx_ignore_import astro mapped\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "app", file)
...
func TestConfigureSkipsOtherDirectives(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := config.New()
	file, err := rule.LoadData("BUILD.bazel", "pkg", []byte("# gazelle:resolve astro astro foo //foo:bar\n# gazelle:dx_ignore_import python foo\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "pkg", file)
...
func TestConfigureThreeFieldAndMalformed(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := config.New()
	file, err := rule.LoadData("BUILD.bazel", "pkg", []byte("# gazelle:dx_ignore_import astro astro mydep\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "pkg", file)
	...
	badCfg := config.New()
	badFile, err := rule.LoadData("BUILD.bazel", "pkg", []byte("# gazelle:dx_ignore_import astro\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestMatchingIgnoreMiss(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := config.New()
	file, err := rule.LoadData("BUILD.bazel", "app", []byte("# gazelle:dx_ignore_import astro other\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "app", file)
