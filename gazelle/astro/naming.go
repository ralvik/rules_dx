// Naming implements the deterministic Astro target-name normalizer owned
// by the first-party Astro Gazelle extension.
//
...
//
// Every supported `.astro` component receives one ordinary
// reusable one-source `astro_library`. Astro tests and entries stay
// JavaScript-owned (parsed or compiled outputs exercised through the
// `javascript_test`/`javascript_binary` wrappers); `.astro` sources are never tests
...
// SupportedExts are the Astro source extensions discovered by the
...
// stripAstroExt reports the basename without its final Astro extension.
...
// TargetName derives the Bazel target name for one Astro source path:
...
// ModuleName returns the import identity for one Astro source path:
