...
func TestGeneratePackageLevelLibrary(t *testing.T) {
	...
	if lib.Kind() != LibraryKind || lib.Name() != "demo" {
		t.Fatalf("library = %s(%s), want dx_kotlin_library(demo)", lib.Kind(), lib.Name())
	}
...
func TestLanguageMetadata(t *testing.T) {
	...
	if len(loads) != 1 || loads[0].Name != "@renamed_dx//kotlin/rules:defs.bzl" || strings.Join(loads[0].Symbols, ",") != "dx_kotlin_library" {
