...
func TestIgnoreWitnessIncludesOnlyUsedUniqueInheritedEntries(t *testing.T) {
	...
	lang := &kotlinLang{}
	f := &rule.File{Directives: []rule.Directive{{Key: "other", Value: "ignored"}, {Key: "dx_ignore_import", Value: "kotlin kotlin Widget"}}}
	...
	entry.used = true
	conf := cfg.Exts[languageName].(*kotlinConfig)
	...
	lang.Configure(cfg, "parent", &rule.File{Directives: []rule.Directive{{Key: "dx_ignore_import", Value: "kotlin"}}})
...
func TestGeneratePackageLevelLibrary(t *testing.T) {
	regular := []string{"Demo.kt", "Helper.kt", "notes.txt", "DemoTest.kt"}
	result := generateFixture(t, map[string]string{
		"pkg/demo/Demo.kt":     "package demo\n\nimport com.example.Widget\n\nclass Demo\n",
		"pkg/demo/Helper.kt":   "package demo\n\nimport java.util.List\n\nclass Helper\n",
		"pkg/demo/notes.txt":   "not a source\n",
		"pkg/demo/DemoTest.kt": "package demo\n\nimport org.junit.Test\n\nclass DemoTest\n",
	}, regular)
	...
	if lib.Kind() != LibraryKind || lib.Name() != "demo" {
		t.Fatalf("library = %s(%s), want kotlin_library(demo)", lib.Kind(), lib.Name())
	}
	if got := strings.Join(lib.AttrStrings("srcs"), ","); got != "Demo.kt,Helper.kt" {
		t.Errorf("library srcs = %q, want Demo.kt,Helper.kt", got)
	}
...
func TestGenerateTestSourcesExcluded(t *testing.T) {
	regular := []string{"Demo.kt", "DemoTest.kt"}
	result := generateFixture(t, map[string]string{
		"pkg/demo/Demo.kt":     "package demo\n\nclass Demo\n",
	...
	if got := strings.Join(result.Gen[0].AttrStrings("srcs"), ","); got != "Demo.kt" {
		t.Errorf("library srcs = %q, want Demo.kt", got)
	}
...
func TestGenerateMainFails(t *testing.T) {
	regular := []string{"Demo.kt", "Main.kt"}
	result := generateFixture(t, map[string]string{
		"pkg/demo/Demo.kt": "package demo\n\nclass Demo\n",
		"pkg/demo/Main.kt": "package demo\n\nobject Main {\n  fun main() {}\n}\n",
	}, regular)
	if len(result.Gen) != 0 {
...
func TestGenerateMixedPackagesFail(t *testing.T) {
	regular := []string{"A.kt", "B.kt"}
	result := generateFixture(t, map[string]string{
		"pkg/demo/A.kt": "package alpha\n\nclass A\n",
		"pkg/demo/B.kt": "package beta\n\nclass B\n",
	}, regular)
	if len(result.Gen) != 0 {
...
func TestImportsIndexNonTestSources(t *testing.T) {
	...
	lib.SetAttr("srcs", []string{"Demo.kt", "Helper.kt", "DemoTest.kt", "notes.txt"})
...
func TestAfterResolvingDepsReportsFailures(t *testing.T) {
	...
	defer func() {
		...
		if msg, ok := r.(string); !ok || !strings.Contains(msg, "Kotlin generation failed") {
			t.Fatalf("panic = %v, want Kotlin generation failure", r)
		}
	}()
...
func TestLanguageMetadata(t *testing.T) {
	l := &kotlinLang{}
	if l.Name() != "kotlin" || len(l.Kinds()) != 1 || l.CheckFlags(flag.NewFlagSet("test", flag.ContinueOnError), config.New()) != nil {
	...
	if len(loads) != 1 || loads[0].Name != "@renamed_dx//kotlin/rules:defs.bzl" || strings.Join(loads[0].Symbols, ",") != "kotlin_library" {
		t.Errorf("apparent loads = %+v", loads)
	}
	if defaults := l.Loads(); len(defaults) != 1 || defaults[0].Name != "@rules_dx//kotlin/rules:defs.bzl" {
		t.Errorf("default loads = %+v", defaults)
	}
	defaultApparent := l.ApparentLoads(func(string) string { return "" })
	if defaultApparent[0].Name != "@rules_dx//kotlin/rules:defs.bzl" {
...
func resolverIndex(lang *kotlinLang, entries ...struct {
	pkg  string
	name string
}) *resolve.RuleIndex {
	index := resolve.NewRuleIndex(func(r *rule.Rule, _ string) resolve.Resolver {
		if _, ok := kotlinKinds[r.Kind()]; ok {
			return lang
		}
		return nil
	})
	for _, entry := range entries {
		...
		r.SetAttr("srcs", []string{entry.name + ".kt"})
...
func TestResolveBranches(t *testing.T) {
	...
	l.Resolve(cfg, index, nil, std, targetImports{imports: []string{"java.util.List"}}, label.New("", "app", "uses_std"))
...
func TestResolveOverride(t *testing.T) {
	l := &kotlinLang{}
	cfg := resolverConfig(t, []rule.Directive{{Key: "resolve", Value: "kotlin kotlin Mapped //mapped:dep"}})
	r := rule.NewRule(LibraryKind, "app")
...
func TestResolveIgnore(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := resolverConfig(t, nil)
	file, err := rule.LoadData("BUILD.bazel", "app", []byte("# gazelle:dx_ignore_import kotlin Dropped\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveStaleIgnoreFails(t *testing.T) {
	...
	l.Before(context.Background())
	cfg := resolverConfig(t, nil)
	file, err := rule.LoadData("BUILD.bazel", "app", []byte("# gazelle:dx_ignore_import kotlin Unused\n"))
	if err != nil {
		t.Fatal(err)
	}
	l.Configure(cfg, "app", file)
