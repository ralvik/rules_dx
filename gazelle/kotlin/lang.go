...
// NewLanguage returns the private first-party Kotlin Gazelle extension.
func NewLanguage() language.Language { return &kotlinLang{} }

func (l *kotlinLang) Before(context.Context) { l.errors = nil; l.ignores = nil }

func (l *kotlinLang) DoneGeneratingRules() {}

func (l *kotlinLang) RegisterFlags(*flag.FlagSet, string, *config.Config) {}

func (l *kotlinLang) CheckFlags(*flag.FlagSet, *config.Config) error { return nil }

func (*kotlinLang) KnownDirectives() []string { return []string{"dx_ignore_import"} }

func (l *kotlinLang) Configure(c *config.Config, rel string, f *rule.File) {
	var inherited []*ignoreEntry
	if raw, ok := c.Exts[languageName]; ok {
		inherited = append(inherited, raw.(*kotlinConfig).ignores...)
	}
	if f != nil {
		for _, directive := range f.Directives {
			...
			if len(fields) == 2 && fields[0] == languageName {
				...
				l.fail("kotlin: //%s: malformed # gazelle:dx_ignore_import %s", rel, directive.Value)
			}
		}
	}
	c.Exts[languageName] = &kotlinConfig{ignores: inherited}
}

func (l *kotlinLang) fail(format string, args ...interface{}) {
	l.errors = append(l.errors, fmt.Sprintf(format, args...))
}

func (l *kotlinLang) AfterResolvingDeps(context.Context) {
	for _, ignore := range l.ignores {
		if !ignore.used {
			l.fail("kotlin: //%s: stale # gazelle:dx_ignore_import kotlin %s matches no literal reference", ignore.path, ignore.value)
		}
	}
	if len(l.errors) == 0 {
		return
	...
	panic("Kotlin generation failed:\n" + strings.Join(l.errors, "\n"))
}

func (*kotlinLang) Name() string { return languageName }

func (*kotlinLang) Kinds() map[string]rule.KindInfo { return kotlinKinds }

func (*kotlinLang) Loads() []rule.LoadInfo {
	return kotlinLoads("rules_dx")
}

func (l *kotlinLang) ApparentLoads(moduleToApparentName func(string) string) []rule.LoadInfo {
	repoName := moduleToApparentName("rules_dx")
	if repoName == "" {
		repoName = "rules_dx"
	}
	return kotlinLoads(repoName)
}

func kotlinLoads(rulesRepo string) []rule.LoadInfo {
	return []rule.LoadInfo{
		{Name: "@" + rulesRepo + "//kotlin/rules:defs.bzl", Symbols: []string{LibraryKind}},
	}
}

// Imports indexes one reusable import identity per Kotlin source owned by a
// library rule: the simple class name of each non-test source. Test-owned
// sources never contribute an identity.
func (*kotlinLang) Imports(_ *config.Config, r *rule.Rule, _ *rule.File) []resolve.ImportSpec {
...
func (*kotlinLang) Embeds(*rule.Rule, label.Label) []label.Label { return nil }

func (l *kotlinLang) GenerateRules(args language.GenerateArgs) language.GenerateResult {
...
func (l *kotlinLang) generateRules(args language.GenerateArgs) language.GenerateResult {
	...
	if err != nil {
		l.fail("kotlin: %s: %v", args.Rel, err)
	...
	for _, src := range sources {
		...
		if err != nil {
			l.fail("kotlin: %s: read %s: %v", args.Rel, src, err)
			continue
		}
		...
		if err != nil {
			l.fail("kotlin: %s: parse package %s: %v", args.Rel, src, err)
			continue
		}
		packages[pkg] = true
	...
	if len(packages) > 1 {
		...
		sort.Strings(names)
		l.fail("kotlin: %s: mixed packages %s in one directory; split the directory before adopting generation", args.Rel, strings.Join(names, ", "))
		return language.GenerateResult{}
	}
...
func mergeStale(file *rule.File, result language.GenerateResult) language.GenerateResult {
	...
	for _, existing := range file.Rules {
		if _, owned := kotlinKinds[existing.Kind()]; !owned || desired[existing.Kind()+"\x00"+existing.Name()] {
			continue
		}
...
func (l *kotlinLang) Resolve(c *config.Config, ix *resolve.RuleIndex, _ *repo.RemoteCache, r *rule.Rule, raw interface{}, from label.Label) {
	...
	for _, name := range imports.imports {
		...
		if override, found := resolve.FindRuleWithOverride(c, spec, languageName); found {
			if ignore := matchingIgnore(c, name); ignore != nil {
				ignore.used = true
				l.fail("kotlin: %s: import %q has both an exact resolve mapping and ignore", from, name)
				continue
			}
		...
		case 0:
			...
			l.fail("kotlin: %s: unresolved import %q; add a local one-source library or an exact # gazelle:resolve mapping", from, name)
		default:
			l.fail("kotlin: %s: ambiguous import %q resolves to %s", from, name, formatMatches(matches))
		}
	}
	if len(deps) == 0 {
		return
...
func matchingIgnore(c *config.Config, name string) *ignoreEntry {
	...
	for i := len(raw.(*kotlinConfig).ignores) - 1; i >= 0; i-- {
		if entry := raw.(*kotlinConfig).ignores[i]; entry.value == name {
			return entry
		}
	}
	return nil
