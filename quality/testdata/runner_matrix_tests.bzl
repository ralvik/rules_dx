...
only `DefaultInfo`, plus analysis-time-written generated bytes) and asserts
the decoded result against an inline snapshot. No `QualitySourcesInfo`
...
Snapshot testing, not brittle equality: the harness first schema-validates
the decoded `print_result` shape (required fields, stage/count consistency),
then byte-compares against the inline golden. With `UPDATE_EXPECT=1` the
harness stages the fresh `actual` under `$TEST_UNDECLARED_OUTPUTS_DIR` and
prints the replacement `expected` block for `runner_matrix_cases.bzl`
instead of failing; without it the harness fails with a unified diff.
...
via the standard `runfiles.bash` `rlocation` (issue #317) with rlocation
strings single-quoted by `shell.quote`; the venv launchers (pydoclint,
flake8, pylint) and Node wrappers (eslint, prettier) ride as tool
binaries with their runfiles merged and resolve their runtimes through
the test `RUNFILES_DIR`, exactly as the `real_aspects.bzl` forests
provide under `dx`. rustfmt resolves from the pinned toolchain via
`rustfmt_from_toolchain`, never from ambient host tools.
"""
