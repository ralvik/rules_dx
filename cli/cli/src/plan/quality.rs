//! Quality and workflow Bazel planning (issue #236).
//!
//! Split from `super` (`plan.rs`): owns the static command registry
//! ([`CommandSpec`]/[`spec`]), the quality required/protected option
//! helpers, [`plan_build`], and the `build`/`test`/`coverage` workflow
//! planning ([`WorkflowVerb`], [`plan_workflow`],
//! [`workflow_scope_labels`]). Re-exported through `super` so the
//! public paths stay `crate::plan::{spec, plan_build, plan_workflow,
//! WorkflowVerb, ...}`. Shares the policy leaves
//! ([`super::workspace_flag`], [`super::BuildPlan`], flag constants)
//! with the generate/run-deploy/managed planning; scope resolution for
