...
`SvelteEnvPlanInfo` is the provider-derived focused-target plan
contribution: direct sources, transitive first-party sources, and npm-closure
size read from the analyzed authoritative providers of one `dx_svelte_*`
...
`svelte_env_plan` materializes that plan as a deterministic JSON file
...
svelte_env_plan = rule(
    ...
    attrs = {
        "target": attr.label(
            mandatory = True,
            doc = "One dx_svelte_* wrapper target to plan (focused target only).",
        ),
    },
