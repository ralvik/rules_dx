...
def _svelte_quality_sources(ctx):
...
def _svelte_preserved_providers(ctx):
    upstream = ctx.attr.upstream
    if _JsInfo not in upstream:
        fail("svelte_*: upstream target has no JsInfo: " + str(ctx.attr.upstream.label))
    return [upstream[_JsInfo]]

def _svelte_forwarded_runtime_providers(ctx):
    upstream = ctx.attr.upstream
    out = []
    if InstrumentedFilesInfo not in upstream:
        fail("svelte_*: upstream target has no InstrumentedFilesInfo: " + str(ctx.attr.upstream.label))
...
def _svelte_library_forward_impl(ctx):
    return (
        _svelte_preserved_providers(ctx) +
        [ctx.attr.upstream[DefaultInfo]] +
        _svelte_forwarded_runtime_providers(ctx) +
        [_svelte_quality_sources(ctx)]
    )

_svelte_library_forward = rule(
    implementation = _svelte_library_forward_impl,
