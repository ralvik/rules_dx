"""Focused C/C++ environment plan (M22 WP2).
...
`cc_env_plan` materializes that plan as a deterministic JSON file plus a
`DxSubjectInfo` observation surface for `starlark_test` analysis mode.
Direct sources come from `QualitySourcesInfo`; the C++ toolchain closure
stays readable from the preserved `CcInfo` without duplicating it here.
