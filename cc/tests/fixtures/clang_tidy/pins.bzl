...
# delegated text diagnostics in Layer-2. Compile-commands context rides
# the authoritative target when present; the adapter never requires it to
# produce findings and never silently skips without it. --fix plus
# --export-fixes plus clang-apply-replacements stay unbound: the runner is
# check-only with the provisional sandbox-apply-and-diff fix flow.
