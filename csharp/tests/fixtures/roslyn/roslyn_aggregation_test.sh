#!/usr/bin/env bash
# Roslyn per-TFM-RID SARIF aggregation wiring test: asserts the per-pivot
# SARIFs prove the union need (shared CA1822 in both, TFM-specific CA1303
# only in net10) and the aggregate concatenates per-pivot runs verbatim in
# deterministic pivot order (ascending TFM version, then RID, then
# configuration) with per-pivot provenance and workspace-relative URIs.
...
# Per-pivot shape: SARIF 2.1, exactly one csc run each.
...
# Union proof: shared CA1822 in both pivots, TFM-specific CA1303 only in
# the net10.0 pivot (a pick-one reader would silently drop it).
...
# Aggregation: single schema plus version, two runs in deterministic pivot
# order (ascending TFM version, then RID, then configuration; plain string
# sort would invert net10/net8), per-pivot runs verbatim, union of three
# results, per-run provenance intact, workspace-relative artifact URIs.
