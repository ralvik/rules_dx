...
//csharp). Each `dx_fsharp_*` macro creates one private
...
def _dx_fsharp_quality_sources(ctx):
...
def _dx_fsharp_preserved_library_providers(ctx):
    upstream = ctx.attr.upstream
    if DotnetAssemblyCompileInfo not in upstream:
        fail("dx_fsharp_*: upstream target has no DotnetAssemblyCompileInfo: " + str(ctx.attr.upstream.label))
    if DotnetAssemblyRuntimeInfo not in upstream:
        fail("dx_fsharp_*: upstream target has no DotnetAssemblyRuntimeInfo: " + str(ctx.attr.upstream.label))
    return [upstream[DotnetAssemblyCompileInfo], upstream[DotnetAssemblyRuntimeInfo]]

def _dx_fsharp_forwarded_output_providers(ctx):
...
def _dx_fsharp_forwarded_assembly_infos(ctx):
...
def _dx_fsharp_forwarded_instrumented(ctx):
...
def _dx_fsharp_library_forward_impl(ctx):
    upstream = ctx.attr.upstream
    return (
        _dx_fsharp_preserved_library_providers(ctx) +
        [upstream[DefaultInfo]] +
        _dx_fsharp_forwarded_instrumented(ctx) +
        _dx_fsharp_forwarded_output_providers(ctx) +
        [_dx_fsharp_quality_sources(ctx)]
    )

_dx_fsharp_library_forward = rule(
    implementation = _dx_fsharp_library_forward_impl,
...
def _dx_fsharp_symlink_default_info(ctx):
    upstream = ctx.attr.upstream[DefaultInfo]
    exe = upstream.files_to_run.executable
    if exe == None:
        fail("dx_fsharp_*: upstream target has no executable: " + str(ctx.attr.upstream.label))
...
def _dx_fsharp_binary_forward_impl(ctx):
    return (
        [_dx_fsharp_symlink_default_info(ctx)] +
        _dx_fsharp_forwarded_assembly_infos(ctx) +
        _dx_fsharp_forwarded_instrumented(ctx) +
        _dx_fsharp_forwarded_output_providers(ctx) +
        [_dx_fsharp_quality_sources(ctx)]
    )

_dx_fsharp_binary_forward = rule(
    implementation = _dx_fsharp_binary_forward_impl,
    executable = True,
    ...
    doc = "Executable forwarder for dx_fsharp_binary: symlinks the upstream binary.",
)

def _dx_fsharp_test_forward_impl(ctx):
    return (
        [_dx_fsharp_symlink_default_info(ctx)] +
        _dx_fsharp_forwarded_assembly_infos(ctx) +
        _dx_fsharp_forwarded_instrumented(ctx) +
...
_dx_fsharp_forward_test = rule(
    implementation = _dx_fsharp_test_forward_impl,
    test = True,
    provides = _DX_FSHARP_EXEC_PROVIDES,
    attrs = {
        "srcs": attr.label_list(
    ...
    doc = "Test forwarder for dx_fsharp_test: symlinks the upstream test executable.",
)

def _dx_fsharp_with_tfm(kwargs):
    upstream_kwargs = dict(kwargs)
    upstream_kwargs.setdefault("target_frameworks", ["net10.0"])
    return upstream_kwargs

def _dx_fsharp_wrap_library(name, srcs, visibility = None, **kwargs):
    _fsharp_library(
        name = name + "_dx_upstream",
        srcs = srcs,
        visibility = ["//visibility:private"],
        **_dx_fsharp_with_tfm(kwargs)
    )
    _dx_fsharp_library_forward(
        name = name,
...
def _dx_fsharp_wrap_binary(name, srcs, visibility = None, **kwargs):
    upstream_kwargs = _dx_fsharp_with_tfm(kwargs)
    if len(srcs) > 0:
...
def dx_fsharp_library(name, srcs, visibility = None, **kwargs):
    ...
    """
    _dx_fsharp_wrap_library(name, srcs, visibility = visibility, **kwargs)

def dx_fsharp_binary(name, srcs = None, visibility = None, **kwargs):
    ...
    _dx_fsharp_wrap_binary(name, effective_srcs, visibility = visibility, **kwargs)

def dx_fsharp_test(name, srcs, visibility = None, **kwargs):
