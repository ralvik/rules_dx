...
ambient compiler discovery. Each library plus test carries one pinned module
dep (`FSharp.Core 10.1.201` via `adopt-fsharp.fsproj` plus
`third_party/dotnet/paket.lock` plus `@paket.main//fsharp.core` with `# keep`
where generation owns the library); the greet and solo tests prove build and
test over the lock. Depcheck `locks` consistency, quality
